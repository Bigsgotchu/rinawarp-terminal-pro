...
exampleRouter.post('/api/users/register', async (req, res) => {
  const { captureErrorWithContext, trackPerformance, addBreadcrumb, wrapRouteHandler } = await import('../src/utils/sentry-helpers.js');
...
exampleRouter.get('/api/analytics/reports/:reportId', async (req, res) => {
  const { captureErrorWithContext, trackPerformance, addBreadcrumb, wrapRouteHandler, monitorDatabaseCall } = await import('../src/utils/sentry-helpers.js');
  ...
  }, res, 'analytics_report_generation');
});

// Example 3: File upload endpoint with comprehensive monitoring
exampleRouter.post('/api/files/upload', async (req, res) => {
  const { captureErrorWithContext, trackPerformance, addBreadcrumb, wrapRouteHandler, monitorApiCall } = await import('../src/utils/sentry-helpers.js');
...
async function processEmailQueue() {
  const { captureErrorWithContext, trackPerformance, addBreadcrumb, setUserContext } = await import('../src/utils/sentry-helpers.js');
...
function createAuthenticationMiddleware() {
  return async (req, res, next) => {
    const { captureErrorWithContext, addBreadcrumb, setUserContext } = await import('../src/utils/sentry-helpers.js');
