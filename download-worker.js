...
function getR2Bucket(env) {
  try {
    ...
    console.error('[download-worker] R2 binding unavailable', error)
    return null
  }
}
...
async function getManifest(env) {
  try {
    ...
    console.error('[download-worker] failed to load release manifest', error)
    return null
  }
}
...
async function serveR2Object(env, objectKey) {
  try {
    ...
    console.error(`[download-worker] failed to serve ${objectKey}`, error)
    return null
  }
}
