...
if (parensStack.length > 0) {
  ...
  parensStack.slice(-10).forEach(p => {
    console.log(`  Line ${p.line}, column ${p.col}: ${lines[p.line - 1].substring(Math.max(0, p.col - 10), p.col + 10)}`);
  });
}
...
if (bracesStack.length > 0) {
  ...
  bracesStack.slice(-10).forEach(b => {
    console.log(`  Line ${b.line}, column ${b.col}: ${lines[b.line - 1].substring(Math.max(0, b.col - 10), b.col + 10)}`);
  });
}
