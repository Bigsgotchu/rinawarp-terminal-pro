...
const patterns = {
  ...
  // Variable Scope
  undeclaredVariable:
    /(?<![\w.])(?!function|class|if|else|for|while|switch|case|try|catch|finally|return|break|continue|throw|new|typeof|instanceof|delete|void|this|super|import|export|from|as|default|const|let|var|await|async|yield|static|get|set|constructor|extends|of|in)([a-zA-Z_$][a-zA-Z0-9_$]*)\s*(?=[=\+\-\*/%<>&|^!~\?:]|\s*\()/g,
  ...
  // Module/Dependency
  circularDependency:
    /require\(['"`]\.{1,2}\/[^'"]+['"`]\)|import\s+.*\s+from\s+['"`]\.{1,2}\/[^'"]+['"`]/g,
  missingImport: /\b(?:require|import)\s*\(['"`][^'"]+['"`]\)/g,

  // Configuration
  hardcodedSecrets:
    /(?:api[_-]?key|secret|password|token|private[_-]?key)\s*[:=]\s*['"`][^'"`]+['"`]/gi,
  hardcodedUrls: /https?:\/\/(?:localhost|127\.0\.0\.1|192\.168\.\d+\.\d+)/g,

  // Resource Handling
  unclosedResources:
    /(?:open|createReadStream|createWriteStream)\s*\([^)]+\)(?![^{]*\.(?:close|end|destroy))/g,
  ...
  inefficientLoops:
    /for\s*\([^)]+in\s+\w+\)|\.forEach\s*\([^)]+\)\s*(?:{[^}]*push\s*\(|=>.*push\s*\()/g,
...
async function scanFile(filePath) {
  try {
    ...
    if (content.includes('rinawarp') || content.includes('RinaWarp')) {
      ...
      const sdkCallPattern =
        /(?:rinawarp|sdk)\.\w+\s*\([^)]*\)(?!\s*\.(?:then|catch)|(?:\s*;?\s*\n\s*)?catch\s*\()/gi;
