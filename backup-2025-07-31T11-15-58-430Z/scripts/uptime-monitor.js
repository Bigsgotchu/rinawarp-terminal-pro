...
const CONFIG = {
  ...
  backupDomains: ['https://rinawarp-terminal.web.app'],
...
async function checkSite(url) {
  try {
    ...
    const hasCorrectContent =
      content.includes('RinaWarp Terminal') && content.includes('Advanced Terminal Emulator');
...
function getUptimeStats() {
  ...
  const uptimePercentage =
    totalChecks > 0 ? ((successfulChecks / totalChecks) * 100).toFixed(2) : 0;
...
async function runMonitoring() {
  ...
  try {
    ...
    if (primaryResult.isUp && primaryResult.hasCorrectContent) {
      ...
      if (lastStatus !== 'up') {
        log(
          `✅ PRIMARY DOMAIN RECOVERED: ${CONFIG.primaryDomain} - Status: ${primaryResult.status} (${primaryResult.responseTime}ms)`,
          'success'
        );
        lastStatus = 'up';
      }
      ...
      if (lastStatus !== 'down') {
        log(
          `❌ PRIMARY DOMAIN DOWN: ${CONFIG.primaryDomain} - Status: ${primaryResult.status} - Error: ${primaryResult.error || 'Unknown'}`,
          'error'
        );
        lastStatus = 'down';
      }
      ...
      if (failureCount >= CONFIG.alertThreshold) {
        sendAlert(
          `Primary domain ${CONFIG.primaryDomain} has been down for ${failureCount} consecutive checks`
        );
      }
    }
    ...
    if (totalChecks % 60 === 0) {
      ...
      log(
        `📊 UPTIME STATS: ${stats.uptimePercentage}% uptime over ${stats.uptimeHours}h (${stats.successfulChecks}/${stats.totalChecks} checks)`,
        'info'
      );
    }
  } catch (error) {
...
process.on('SIGINT', () => {
  const stats = getUptimeStats();
  log(
    `🏁 Monitoring stopped. Final stats: ${stats.uptimePercentage}% uptime over ${stats.uptimeHours}h`,
    'info'
  );
  process.exit(0);
});
