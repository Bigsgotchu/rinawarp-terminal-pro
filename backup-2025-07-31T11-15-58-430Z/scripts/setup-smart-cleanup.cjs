...
class SmartCleanupSetup {
  ...
  async checkExistingSetup() {
    ...
    for (const [key, filePath] of Object.entries(files)) {
      ...
      this.log(
        `  ${status[key] ? '✅' : '❌'} ${key}: ${filePath}`,
        status[key] ? 'success' : 'warning'
      );
    }
...
`;
  ...
  async runInitialCleanup() {
    const shouldRun =
      process.argv.includes('--run-cleanup') || process.argv.includes('--initial-cleanup');
    ...
    if (shouldRun) {
      ...
      try {
        ...
        const cleanup = spawn(
          'node',
          ['scripts/smart-file-manager.cjs', '--verbose', '--no-watch'],
          {
            stdio: 'inherit',
  ...
  async run() {
    // console.clear(); // Disabled for linting
