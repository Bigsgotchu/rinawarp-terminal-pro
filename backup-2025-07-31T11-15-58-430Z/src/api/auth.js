...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    logger.info(`[AUTH] Generating token for user: ${email} with role: ${role}`);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    logger.info(`[LICENSE] Validation request for key: ${licenseKey.substring(0, 10)}...`);
    ...
    logger.info(`[LICENSE] Valid license found: ${license.type}`);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      logger.error('Error sending test license email:', error);
      res.status(500).json({
