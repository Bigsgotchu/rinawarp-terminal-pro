...
async function initializeReact() {
  ...
  try {
    ...
    if (window.electronAPI && window.electronAPI.React && window.electronAPI.ReactDOM) {
      console.log('✅ Using React from Electron preload');
    ...
    console.log('✅ React authentication UI initialized successfully');
...
class AdvancedGitPlugin {
  ...
  updateGitUI(branchInfo, statusInfo, _remoteInfo) {
...
class SessionManager {
  ...
  createSession(name, description = '') {
    const sessionId = `session_${Date.now()}_${this.generateSecureId()}`;
    const session = {
      id: sessionId,
  ...
  importSession(sessionData) {
    ...
    session.id = `imported_${Date.now()}_${this.generateSecureId()}`;
...
// import { AdvancedAIContextEngine } from './ai-context-engine.js';
...
class _CodeReviewAI {
  ...
  async analyzeCode(_content, _context) {
...
class _PredictiveCommandAI {
  constructor(engine) {
    this.engine = engine;
  }

  async generatePredictions(_command, _userPattern, _context) {
...
class _ErrorAnalysisAI {
  constructor(engine) {
    this.engine = engine;
  }

  async analyzeError(_errorData) {
...
class _DocumentationAI {
  constructor(engine) {
    this.engine = engine;
  }

  async generateDocs(_command, _context) {
...
class TerminalManager extends SimpleEventEmitter {
  ...
  setupWorkflowHandlers(modal) {
    ...
    modal.querySelector('#start-recording')?.addEventListener('click', async () => {
      ...
      if (this.workflowEngine) {
        try {
          await this.workflowEngine.startMacroRecording(name);
    ...
    modal.querySelectorAll('.workflow-template').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (this.workflowEngine) {
          try {
            await this.workflowEngine.discoverWorkflows('deployment');
  ...
  setupAdvancedUIHandlers(modal) {
    ...
    modal.querySelector('#visualize-flow')?.addEventListener('click', async () => {
      ...
      if (command && this.nextGenUI) {
        try {
          await this.nextGenUI.visualizeCommandFlow(command);
    ...
    if (authBtn) {
      authBtn.addEventListener('click', () => {
        ...
        if (window.authModalState) {
          ...
          console.log('⚠️ Auth modal state not initialized yet');
          this.pluginAPI.showNotification(
  ...
  setupSettingsModal() {
    ...
    const autoUpdateEnabledCheck = document.getElementById('auto-update-enabled');
    const checkUpdatesBtn = document.getElementById('check-updates-btn');
    const updateStatus = document.getElementById('update-status');

    if (autoUpdateEnabledCheck) {
      autoUpdateEnabledCheck.checked = this.settings.autoUpdateEnabled !== false; // Default to true
      autoUpdateEnabledCheck.addEventListener('change', () => {
        this.settings.autoUpdateEnabled = autoUpdateEnabledCheck.checked;
        this.saveSettings();
        this.pluginAPI.showNotification(
          autoUpdateEnabledCheck.checked ? 'Auto-updates enabled' : 'Auto-updates disabled',
          'success',
          3000
        );
      });
    ...
    if (checkUpdatesBtn) {
      checkUpdatesBtn.addEventListener('click', async () => {
        ...
        try {
          // Call the IPC handler to check for updates
          ...
          setTimeout(() => {
            ...
            if (result && result.updateInfo) {
              updateStatus.textContent = `Update available: v${result.updateInfo.version}`;
              this.pluginAPI.showNotification(
                'Update available! Download will start automatically.',
                'success',
                5000
              );
            } else {
              updateStatus.textContent = 'Version 1.0.7 (Current - Up to date)';
              this.pluginAPI.showNotification(
          ...
          }, 2000); // Give some time for the update check to complete
        } catch (error) {
  ...
  // Wrapper methods for AI Assistant integration
  insertCommand(command) {
    const terminalData = this.terminals.get(this.activeTerminalId);
  ...
  adjustFontSize(delta) {
    this.settings.fontSize = Math.max(8, Math.min(32, this.settings.fontSize + delta));
    this.applyFontSizeToTerminals();
    this.saveSettings();
  }
  ...
  // Safe wrapper for accessing terminal methods
  getActiveTerminalInstance() {
    const terminalData = this.terminals.get(this.activeTerminalId);
    return terminalData ? terminalData.terminal : null;
  }

  // Safe wrapper for accessing fit addon
  getActiveTerminalFitAddon() {
    const terminalData = this.terminals.get(this.activeTerminalId);
    return terminalData ? terminalData.fitAddon : null;
  }
  ...
  setupLicenseActions(modal) {
    ...
    modal.querySelector('#view-pricing')?.addEventListener('click', () => {
      ...
      const pricingUrl = process.env.PRICING_URL || 'https://example.com/pricing';
      import('electron').then(electron => electron.shell.openExternal(pricingUrl));
    });
