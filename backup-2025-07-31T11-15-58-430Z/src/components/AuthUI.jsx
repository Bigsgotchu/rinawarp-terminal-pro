...
export function AuthUI({ onAuthSuccess }) {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
      const hasSession = await enhancedAuthService.checkStoredSession();
      if (hasSession) {
        setIsAuthenticated(true);
        if (onAuthSuccess) {
          onAuthSuccess(enhancedAuthService.getCurrentUser());
        }
      }
    };
    checkSession();
  ...
  const handleSubmit = async e => {
    ...
    try {
      let result;
      if (mode === 'login') {
        result = await enhancedAuthService.signIn(email, password);
      } else {
        result = await enhancedAuthService.signUp(email, password, displayName);
      }

      if (result.success) {
        // Save session
        enhancedAuthService.saveSession();
