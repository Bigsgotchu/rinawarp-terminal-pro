...
class BrowserLinkTester {
    ...
    async testLink(link, templateName, deviceName) {
        try {
            ...
            const hasErrorPage = pageContent.includes('404') || 
                                pageContent.includes('Page not found') ||
                                pageContent.includes('Error');
            
            // Check for payment-related elements if it's a payment link
            const isPaymentLink = link.url.includes('stripe') || 
            ...
            console.log(`      ${status} ${result.response.status} - ${result.response.statusText} (${loadTime}ms)`);
    ...
    async checkPaymentElements() {
        try {
            const elements = await this.page.evaluate(() => {
                return {
                    hasStripeElements: document.querySelector('[data-stripe]') !== null ||
                                     document.querySelector('.stripe-button') !== null ||
                                     document.querySelector('#stripe-payment-element') !== null,
                    hasPaymentForm: document.querySelector('form[action*="payment"]') !== null ||
                                   document.querySelector('form[action*="checkout"]') !== null,
                    hasSecureIndicators: document.querySelector('[data-secure]') !== null ||
    ...
    async checkDownloadElements() {
        try {
            const elements = await this.page.evaluate(() => {
                return {
                    hasDownloadButton: document.querySelector('a[download]') !== null ||
                    ...
                    hasFileLinks: document.querySelectorAll('a[href$=".exe"], a[href$=".dmg"], a[href$=".zip"], a[href$=".deb"]').length
                };
            });
        ...
        for (const templatePath of this.emailTemplates) {
            ...
            try {
                ...
                for (const device of this.devices) {
                    ...
                    // Take screenshot
                    const screenshotPath = path.join(__dirname, `screenshots/${templateName}-${device.name}.png`);
    ...
    async generateReport() {
        ...
        Object.keys(resultsByTemplate).forEach(template => {
            ...
            Object.keys(resultsByTemplate[template]).forEach(device => {
                ...
                console.log(`  📱 ${device}: ${deviceSuccessful}/${deviceTotal} (${((deviceSuccessful/deviceTotal)*100).toFixed(1)}%)`);
                ...
                if (failedResults.length > 0) {
                    ...
                    failedResults.forEach(result => {
                        ...
                        console.log(`        ${result.error || (result.response && result.response.statusText)}`);
                    });
                }
            });
        });
        ...
        if (performanceResults.length > 0) {
            const avgLoadTime = performanceResults.reduce((sum, r) => sum + r.response.loadTime, 0) / performanceResults.length;
