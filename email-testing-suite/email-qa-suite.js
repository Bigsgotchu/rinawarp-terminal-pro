...
const links = [];
            ...
            try {
                ...
                devices.forEach(device => {
                    ...
                    if (responsiveAnalysis.inlineStyles > 20) {
                        ...
                        deviceTest.recommendations.push('Consider using CSS classes for better maintainability');
                    }
                ...
                console.log(`  📏 Percentage Widths: ${responsiveAnalysis.hasPercentageWidths ? '✅' : '❌'}`);
    ...
    generateLinkReport() {
        ...
        console.log(`Successful: ${successfulLinks} (${((successfulLinks/totalLinks)*100).toFixed(1)}%)`);
    ...
    generateFinalReport() {
        ...
        const summary = {
            ...
            personalization: {
                ...
                percentage: personalizationTotal > 0 ? ((personalizationSuccess / personalizationTotal) * 100).toFixed(1) : 0
            },
            attachments: {
                success: attachmentSuccess,
                total: attachmentTotal,
                percentage: attachmentTotal > 0 ? ((attachmentSuccess / attachmentTotal) * 100).toFixed(1) : 0
            }
        };
        
        console.log('\n📊 SUMMARY STATISTICS:');
        console.log(`🔗 Link Verification: ${summary.linkVerification.success}/${summary.linkVerification.total} (${summary.linkVerification.percentage}%)`);
        console.log(`🎯 Personalization: ${summary.personalization.success}/${summary.personalization.total} (${summary.personalization.percentage}%)`);
        console.log(`📎 Attachments: ${summary.attachments.success}/${summary.attachments.total} (${summary.attachments.percentage}%)`);
        
        // Spam score summary
        const spamScores = Object.values(this.testResults.spamScore);
        if (spamScores.length > 0) {
            const avgSpamScore = spamScores.reduce((sum, score) => sum + score.score, 0) / spamScores.length;
        ...
        const overallScore = (
            ...
            (spamScores.length > 0 ? (1 - (spamScores.reduce((sum, score) => sum + score.score, 0) / spamScores.length / 10)) : 0.8) * 0.2
        ) * 100;
