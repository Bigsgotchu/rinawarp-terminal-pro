...
var PRICE_IDS = {
  // Lifetime (one-time)
  TEAM_LIFETIME: "price_1SdxmFGZrRdZy3W9skXi3jvE",
  // $999
  PIONEER_LIFETIME: "price_1Sdxm2GZrRdZy3W9C5tQcWiW",
  // $800
  PRO_LIFETIME: "price_1SdxlmGZrRdZy3W9ncwPfgFr",
  // $699
  // Subscriptions (recurring)
  TEAM_MONTHLY: "price_1SdxlXGZrRdZy3W9Wr1XLBIe",
  // $99/mo
  CREATOR_MONTHLY: "price_1SdxlKGZrRdZy3W9TvaLugc7",
  // $69/mo
  PRO_MONTHLY: "price_1Sdxl7GZrRdZy3W9INQvidPf"
  // $29/mo
};
var ALLOWLIST = new Set(Object.values(PRICE_IDS));
var SUBSCRIPTION_PRICES = /* @__PURE__ */ new Set([
...
async function handleCheckout(request, env) {
  ...
  const customer_email = body?.customer_email ? String(body.customer_email).trim() : void 0;
  if (!price_id) {
  ...
  const stripe = new stripe_esm_worker_default(env.STRIPE_SECRET_KEY, { apiVersion: "2025-02-24.acacia" });
  const mode = SUBSCRIPTION_PRICES.has(price_id) ? "subscription" : "payment";
  try {
...
__name(randomCode6, "randomCode6");
function sleep(ms) {
...
async function authVerify(request, env) {
  ...
  const fail = /* @__PURE__ */ __name(async () => {
    const elapsed2 = Date.now() - startedAt;
    if (elapsed2 < MIN_MS) await sleep(MIN_MS - elapsed2);
    return Response.json({ ok: false, error: "invalid_code" }, { status: 400, headers: corsHeaders });
  }, "fail");
  let email = "";
  let code = "";
  ...
  const user = await env.DB.prepare("SELECT customer_id FROM users WHERE email = ?").bind(email).first();
  ...
  const expiresAt = nowMs() + 7 * 24 * 60 * 60 * 1e3;
  ...
  ).bind(tokenHash, email, user.customer_id, expiresAt, nowMs()).run();
  const elapsed = Date.now() - startedAt;
  if (elapsed < MIN_MS) await sleep(MIN_MS - elapsed);
...
async function requireSession(request, env) {
  ...
  if (parts.length !== 3) return { ok: false, status: 401, error: "invalid_session" };
  const tokenPlain = atob(parts[1]);
  const sig = parts[2];
  const expectedSig = await hmacHex(env.AUTH_TOKEN_SECRET, tokenPlain);
  if (sig !== expectedSig) return { ok: false, status: 401, error: "bad_signature" };
  ...
  if (nowMs() > expiresAt) return { ok: false, status: 401, error: "session_expired" };
  const tokenHash = await sha256Hex(token);
  const row = await env.DB.prepare("SELECT email, customer_id, expires_at FROM sessions WHERE token_hash = ?").bind(tokenHash).first();
  if (!row) return { ok: false, status: 401, error: "session_not_found" };
  if (nowMs() > row.expires_at) return { ok: false, status: 401, error: "session_expired" };
  return { ok: true, email: row.email, customer_id: row.customer_id };
}
...
// src/index.ts
function resolveTier(env, priceId) {
...
var index_default = {
  async fetch(request, env) {
    ...
    if (request.method === "OPTIONS" && (url.pathname === "/api/auth/start" || url.pathname === "/api/auth/verify" || url.pathname === "/api/auth/logout")) {
    ...
    if (url.pathname === "/api/license/verify" && request.method === "POST") {
      ...
      if (!ent) return json2({ ok: false, error: "no_entitlement" }, 403);
      if (ent.status !== "active") return json2({ ok: false, error: "not_active", status: ent.status }, 403);
    ...
    if (url.pathname === "/api/me" && request.method === "GET") {
      ...
      if (env.STRIPE_SECRET_KEY && env.PORTAL_RETURN_URL) {
        try {
          const stripe2 = new stripe_esm_worker_default(env.STRIPE_SECRET_KEY, { apiVersion: "2025-02-24.acacia" });
          const portal = await stripe2.billingPortal.sessions.create({
    ...
    if (url.pathname === "/api/download-token" && request.method === "GET") {
      ...
      return json2({ token, expires_at: expiresAt, tier: ent.tier });
    }
      ...
      case "checkout.session.completed": {
        ...
        try {
          await upsertEntitlement(env, {
            ...
            subscription_id: isRecurring ? session.subscription : null
          });
          ...
          if (!isRecurring) {
            ...
            await env.DB.prepare(
              ...
              VALUES (?, ?, ?, ?, ?, ?)`
            ).bind(licenseKey, customerId, tier, "active", 3, 0).run();
      ...
      case "customer.subscription.deleted": {
        ...
        if (customerId) {
          try {
            ...
            ).bind("revoked", Date.now(), customerId).run();
...
async function upsertEntitlement(env, row) {
  const existing = await env.DB.prepare(
    "SELECT tier, status FROM entitlements WHERE customer_id = ? LIMIT 1"
  ).bind(row.customer_id).first();
  const finalTier = existing && existing.tier ? tierRank(row.tier) > tierRank(existing.tier) ? row.tier : existing.tier : row.tier;
  await env.DB.prepare(
...
async function upsertUserByEmail(env, email, customerId) {
