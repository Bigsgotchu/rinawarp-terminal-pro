...
function docMatchesFilters(doc: IndexDoc, f: SearchFilters): boolean {
  ...
  if (f.cwd && !String(doc.cwd || "").toLowerCase().includes(f.cwd)) return false;
  if (f.session && !String(doc.session_id || "").toLowerCase().includes(f.session)) return false;
...
export class StructuredSessionStore {
  ...
  exportRunbookMarkdown(sessionId?: string): string {
    ...
    const raw = fs.readFileSync(commandsPath, "utf8").split("\n").map((line) => line.trim()).filter(Boolean);
  ...
  private loadSearchIndex(): InvertedIndex {
    ...
    const parsed = fs.existsSync(file)
      ? JSON.parse(fs.readFileSync(file, "utf8")) as InvertedIndex
      : null;
