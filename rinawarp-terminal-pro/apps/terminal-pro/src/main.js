...
// License tier (could be dynamic based on license check)
const CURRENT_LICENSE = "pro";
// Playbook library
const PLAYBOOKS = [
    {
        id: "running-hot",
...
async function diagnoseHotLinux() {
    ...
    const topProcesses = await runCommandOnce("ps -eo pid,pcpu,pmem,comm --sort=-pcpu | head -n 15", 8000).catch((e) => `Unable to read processes: ${String(e)}`);
 ...
 */
function buildStepsForKind(kind, projectRoot) {
...
ipcMain.handle("license:verify", async (_event, customerId) => {
    return verifyLicense(customerId);
});
...
ipcMain.handle("rina:plan", async (_event, intent) => {
...
ipcMain.handle("rina:playbook:execute", async (_event, playbookId, fixIndex) => {
...
ipcMain.handle("rina:transcript:export", async (_event, format) => exportTranscript(format));
ipcMain.handle("rina:transcript:add", async (_event, entry) => addTranscriptEntry({ ...entry, timestamp: new Date().toISOString() }));
ipcMain.handle("rina:memory:get", async (_event, category) => operationalMemory.getRecent(category));
ipcMain.handle("rina:memory:set", async (_event, category, key, value) => {
    operationalMemory.set(category, key, value);
    addTranscriptEntry({ type: "memory", timestamp: new Date().toISOString(), category, key, value });
});
ipcMain.handle("rina:executeStepStream", async (event, step, confirmed, confirmationText, projectRoot) => {
    ...
    // Fire and forget; UI listens to stream events via emit callbacks
    void startStreamingStepViaEngine({
...
ipcMain.handle("rina:stream:cancel", async (_event, streamId) => cancelStream(streamId));
...
ipcMain.handle("rina:plan:stop", async (_event, planRunId) => {
...
ipcMain.handle("rina:doctor:inspect", async (_event, intent) => {
    return await doctorInspect(intent);
});
ipcMain.handle("rina:doctor:collect", async (_event, steps, streamCallback) => {
    // Create a stream callback for IPC
    const wrappedCallback = (chunk, stream) => {
        if (streamCallback) {
...
ipcMain.handle("rina:doctor:interpret", async (_event, payload) => {
    return await doctorInterpret(payload);
});
ipcMain.handle("rina:doctor:verify", async (_event, payload) => {
    return await doctorVerify(payload);
});
ipcMain.handle("rina:doctor:executeFix", async (_event, plan, confirmed, confirmationText) => {
...
ipcMain.handle("rina:doctor:transcript:export", async (_event, format) => doctorExportTranscript(format));
...
function formatDiagnosisForChat(diagnosis) {
    ...
    if (diagnosis.differential?.length) {
        msg += `\n\n**Other possibilities**: ${diagnosis.differential.slice(0, 3).map((d) => `${d.label} (${Math.round(d.probability * 100)}%)`).join(", ")}`;
    }
    return msg;
}
...
ipcMain.handle("rina:chat:send", async (_event, text) => {
...
ipcMain.handle("agent:plan", async (_event, intent) => {
    const plan = makePlan(intent);
    return plan;
});
// ============================================================
...
ipcMain.handle("rina:agent:plan", async (_event, args) => {
    ...
    // v1 heuristic plan (similar to makePlan but with PlanStep format)
...
ipcMain.handle("rina:executePlanStream", async (event, args) => {
    ...
    try {
        for (const step of args.plan) {
            ...
            // Notify renderer to create a step block
            ...
            // Execute step - stream end will be sent to renderer
            ...
            // Clear current stream id after completion
...
ipcMain.handle("rina:doctor:plan", async (_event, args) => {
