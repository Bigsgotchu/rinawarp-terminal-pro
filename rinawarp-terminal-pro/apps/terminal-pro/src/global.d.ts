...
declare global {
  ...
  interface Window {
    rina: {
      ...
      structuredSearch(query: string, limit?: number): Promise<Array<{
        sessionId: string;
        commandId: string;
        command: string;
        cwd?: string;
      ...
      unifiedSearch(query: string, limit?: number): Promise<Array<{
        id: string;
      ...
      reportRendererError(payload: { kind?: string; message?: string; extra?: string }): Promise<{ ok: boolean; error?: string }>;
      ...
      teamGet(): Promise<{ currentUser?: string; members: Array<{ email: string; role: "owner" | "operator" | "viewer" }> }>;
      teamSetCurrentUser(email: string): Promise<{ ok: boolean; error?: string; role?: "owner" | "operator" | "viewer" }>;
      teamUpsertMember(member: { email: string; role: "owner" | "operator" | "viewer" }): Promise<{ ok: boolean; error?: string }>;
