...
exports.planHotComputer = planHotComputer;
 ...
 */
function planHotComputer() {
    const platform = (0, terminal_1.getPlatform)();
    const steps = [
        // Step 1: System info
        {
            tool: "terminal",
 ...
 */
function getThermalSteps(platform) {
    switch (platform) {
        case "darwin":
            return [
                {
 ...
 */
function parseDiagnostics(results) {
    const result = {
        highCpuProcesses: [],
    ...
    if (result.highCpuProcesses.length > 0) {
        result.recommendations.push(`High CPU usage detected from: ${result.highCpuProcesses.join(", ")}. Consider closing or restarting these processes.`);
    }
    if (result.thermalWarning) {
        result.recommendations.push("High temperatures detected. Consider: closing resource-heavy apps, ensuring proper ventilation, or resetting SMC (macOS).");
    }
    if (result.diskFull) {
        result.recommendations.push("Disk is nearly full. Consider clearing cache files, removing unused applications, or clearing temporary files.");
    }
    if (result.recommendations.length === 0) {
        result.recommendations.push("No obvious issues detected. If the computer is still running hot, consider: checking for dust buildup, ensuring vents are clear, or checking for malware.");
    }
    return result;
}
