...
export function configureSecurityControls(args: {
  ...
  if (Number.isFinite(args.evidence_interval_sec)) state.evidence_interval_sec = Math.max(60, Number(args.evidence_interval_sec));
  saveState(state);
  return state;
}
  ...
  if (!force && state.last_drill_at) {
    ...
    if (Number.isFinite(last) && Date.now() - last < state.evidence_interval_sec * 1000) {
      ...
      return {
        ...
        mtls: { mode: state.mtls_mode, provider: state.mesh_provider, ok: state.mtls_mode === "strict" && state.mesh_provider !== "none" },
