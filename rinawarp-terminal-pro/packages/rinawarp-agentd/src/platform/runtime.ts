...
const backend = String(process.env.RINAWARP_RUNTIME_BACKEND || "local").trim().toLowerCase();
...
async function runTaskK8s(task: RuntimeTask): Promise<void> {
  ...
  const initialDelaySec = Math.max(1, Number(task.initial_delay_sec || process.env.RINAWARP_RUNTIME_INITIAL_DELAY_SEC || 10));
...
export function enqueueRuntimeTask(args: {
  ...
  const task: RuntimeTask = {
    ...
    initial_delay_sec: Math.max(1, Number(args.initial_delay_sec || process.env.RINAWARP_RUNTIME_INITIAL_DELAY_SEC || 10)),
...
export function reconcileRuntimeTasks(args?: {
  ...
  const state = loadState();
  const now = Date.now();
  const queueStuckAfterSec = Math.max(60, Number(args?.queueStuckAfterSec || process.env.RINAWARP_RUNTIME_QUEUE_STUCK_AFTER_SEC || 15 * 60));
  const runningStuckGraceSec = Math.max(30, Number(args?.runningStuckGraceSec || process.env.RINAWARP_RUNTIME_RUNNING_STUCK_GRACE_SEC || 5 * 60));
  const autoRemediate = args?.autoRemediate ?? (String(process.env.RINAWARP_RUNTIME_AUTO_REMEDIATE || "true").trim().toLowerCase() === "true");
  const maxRemediations = Math.max(1, Number(args?.maxRemediations || process.env.RINAWARP_RUNTIME_MAX_REMEDIATIONS || 50));
  ...
  for (let i = 0; i < state.tasks.length; i += 1) {
    ...
    const stuckRunning = (task.status === "running" || task.status === "retrying") && runAgeSec > timeoutSec + runningStuckGraceSec;
