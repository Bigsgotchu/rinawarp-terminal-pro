...
function detectConflict(base: VersionVector, current: VersionVector, writer: RegionId): Array<{ region: RegionId; base: number; current: number }> {
...
export function configureActiveActive(args: {
  enabled?: boolean;
  strict_conflicts?: boolean;
}): { enabled: boolean; strict_conflicts: boolean; updated_at: string } {
...
export function activeActiveWrite(args: {
  ...
  | { ok: false; error: "invalid_region" | "disabled" | "conflict"; conflicts?: Array<{ region: RegionId; base: number; current: number }>; current_vector?: VersionVector; replay_from?: VersionVector } {
  const state = loadState();
...
export function runReplicationDrill(args?: {
  workspace_id?: string;
}): {
  ok: boolean;
  checked: number;
  ...
  const details = ids.map((workspaceId) => {
    ...
    const drift = Number(stored["us-east-1"] || 0) !== Number(replay["us-east-1"] || 0) || Number(stored["eu-west-1"] || 0) !== Number(replay["eu-west-1"] || 0);
    return {
...
export function replayWorkspaceEvents(args: {
  workspace_id: string;
  ...
  const startIdx = args.from_event_id ? Math.max(0, events.findIndex((e) => e.event_id === args.from_event_id)) : 0;
