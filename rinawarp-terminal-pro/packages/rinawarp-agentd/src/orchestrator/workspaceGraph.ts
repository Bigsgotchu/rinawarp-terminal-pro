...
export function createIssueToPrWorkflow(input: {
  ...
  putNode(graph, { id: workflowNodeId, type: "workflow", label: `Issue->PR ${input.issueId}`, data: { workflowId }, createdAt: now });
  putNode(graph, { id: issueNodeId, type: "issue", label: `Issue ${input.issueId}`, data: { issueId: input.issueId }, createdAt: now });
  ...
  putNode(graph, { id: prNodeId, type: "pull_request", label: `PR for ${input.issueId}`, data: { status: "planned" }, createdAt: now });
...
export function recordCiStatus(input: {
  ...
  const workflowState =
    input.status === "failed" ? "blocked" : input.status === "passed" ? "verified" : "active";
