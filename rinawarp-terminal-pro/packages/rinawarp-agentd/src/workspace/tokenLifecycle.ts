...
export function configureTokenLifecycle(args: {
  revocation_window_sec?: number;
  refresh_max_age_sec?: number;
}) {
  const state = loadState();
  if (Number.isFinite(args.revocation_window_sec)) state.revocation_window_sec = Math.max(60, Number(args.revocation_window_sec));
  if (Number.isFinite(args.refresh_max_age_sec)) state.refresh_max_age_sec = Math.max(60 * 60, Number(args.refresh_max_age_sec));
  prune(state);
  saveState(state);
  return {
...
export function revokeRefreshSession(args: {
  jti: string;
  reason?: string;
}): { ok: boolean } {
  const state = loadState();
