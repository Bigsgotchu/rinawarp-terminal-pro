...
export function verifySignedAuthToken(token, secret) {
    const parts = String(token || "").split(".");
    if (parts.length !== 3)
        return null;
    const [h, p, s] = parts;
    const expected = signParts(h, p, secret);
    if (!secureCompare(expected, s))
        return null;
    try {
        const parsed = JSON.parse(b64urlDecode(p).toString("utf8"));
        if (!parsed || !parsed.sub || !parsed.email || !parsed.kind || !parsed.exp || !parsed.iat)
            return null;
        if (!["access", "refresh"].includes(parsed.kind))
            return null;
        if (!["owner", "admin", "member"].includes(parsed.role))
            return null;
...
export function parseAuthClaims(req) {
...
export function requireAuth(req, opts) {
    ...
    if (!required) {
        ...
        if (!process.env.RINAWARP_AGENTD_AUTH_SECRET)
            return;
