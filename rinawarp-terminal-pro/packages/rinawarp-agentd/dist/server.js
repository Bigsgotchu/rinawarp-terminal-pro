...
async function readRawJson(req, opts) {
    const chunks = [];
...
function webhookAuditFile() {
    return path.join(paths().baseDir, "webhook-audit.ndjson");
}
async function appendWebhookAudit(event) {
...
function readWebhookAudit(args) {
    ...
    const filtered = parsed.filter((entry) => {
        if (outcome && String(entry.outcome || "") !== outcome)
            return false;
        if (mapped && String(entry.mapped || "") !== mapped)
            return false;
        return true;
    });
...
function checkWebhookRateLimit(clientIp, now = Date.now()) {
    ...
    const server = http.createServer(async (req, res) => {
        try {
            ...
            if (req.method === "PUT" && url.pathname === "/v1/platform/regions/health") {
                ...
                if (!region || !status)
                    return sendJson(res, 400, { ok: false, error: "region and status are required" });
                const updated = setRegionHealth(region, status);
                if (!updated)
                    return sendJson(res, 400, { ok: false, error: "invalid_region" });
                logSoc2({ req, action: "region_health_set", result: "ok", details: updated });
            ...
            if (req.method === "GET" && url.pathname === "/v1/daemon/status") {
                ...
                const counts = registry.tasks.reduce((acc, task) => {
                    acc[task.status] = (acc[task.status] ?? 0) + 1;
                    if (task.deadLetter)
                        acc.dead_letter = (acc.dead_letter ?? 0) + 1;
                    return acc;
                }, {});
                return sendJson(res, 200, {
                    ok: true,
            ...
            if (req.method === "POST" && url.pathname === "/v1/daemon/start") {
                ...
                if (!start.ok)
                    return sendJson(res, 500, { ok: false, error: start.error || "failed to start daemon" });
                return sendJson(res, 200, {
                    ok: true,
            ...
            if (req.method === "POST" && url.pathname === "/v1/daemon/stop") {
                ...
                if (!stop.ok)
                    return sendJson(res, 500, { ok: false, error: stop.error || "failed to stop daemon" });
                return sendJson(res, 200, {
                    ok: true,
                    stopped: !stop.stale,
                    stale: !!stop.stale,
            ...
            if (req.method === "GET" && url.pathname === "/v1/daemon/tasks") {
                ...
                const tasks = registry.tasks.filter((task) => {
                    if (status && task.status !== status)
                        return false;
                    if (deadOnly && task.deadLetter !== true)
                        return false;
                    return true;
                });
            ...
            if (req.method === "POST" && url.pathname === "/v1/daemon/tasks") {
                ...
                if (!type)
                    return sendJson(res, 400, { ok: false, error: "type is required" });
            ...
            if (req.method === "POST" && url.pathname === "/v1/orchestrator/issue-to-pr") {
                ...
                if (!issueId)
                    return sendJson(res, 400, { ok: false, error: "issueId is required" });
                if (!repoPath)
                    return sendJson(res, 400, { ok: false, error: "repoPath is required" });
