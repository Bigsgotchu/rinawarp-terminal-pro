...
export class DataRecoveryUI {
  ...
  async loadDrives() {
    try {
      ...
      driveList.innerHTML = drives.map(drive => {
        const sizeGB = drive.totalSize ? Math.round(drive.totalSize / (1024 * 1024 * 1024)) : 'Unknown';
        const freeGB = drive.freeSpace ? Math.round(drive.freeSpace / (1024 * 1024 * 1024)) : 'Unknown';
      ...
      document.getElementById('driveList').innerHTML = '<div class="error">Error loading drives</div>';
    }
  }
  ...
  async startScan() {
    ...
    const fileTypes = Array.from(document.querySelectorAll('.file-type-filters input:checked'))
      .map(cb => cb.value);
  ...
  displayFileList(files) {
    ...
    fileList.innerHTML = files.map((file, index) => {
      ...
      const chanceColor = file.recoveryChance >= 70 ? 'high' : file.recoveryChance >= 50 ? 'medium' : 'low';
