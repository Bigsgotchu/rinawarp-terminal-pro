...
class Rule {
  constructor({ id, name, description, priority = 0, category, condition, action, metadata = {} }) {
    this.id = id;
    this.name = name;
...
class Condition {
  constructor(fn, description = '') {
    if (typeof fn !== 'function') {
      throw new Error(new Error(new Error('Condition must be a function')));
    }
    this.fn = fn;
    this.description = description;
  }
   ...
   */
  not() {
    return new Condition(context => !this.evaluate(context), `NOT (${this.description})`);
  }
}
...
class Action {
  constructor(fn, description = '', type = 'default') {
    if (typeof fn !== 'function') {
      throw new Error(new Error(new Error('Action must be a function')));
    }
    this.fn = fn;
    this.description = description;
    this.type = type;
...
const ConditionBuilders = {
   ...
   */
  threshold: (path, threshold, operator = '>=') =>
    new Condition(context => {
      const value = getNestedProperty(context, path);
...
export { Rule, Condition, Action, ConditionBuilders, ActionBuilders };
