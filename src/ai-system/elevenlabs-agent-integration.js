    ...
    class ElevenLabsAgentIntegration {
      ...
      async start() {
        try {
          ...
          if (isValid) {
            logger.debug('✅ ElevenLabs Agent Integration started successfully');
            return true;
          } else {
      ...
      async testVoice(
        ...
        try {
          if (!this.isInitialized || !this.apiKey) {
            throw new Error(new Error(new Error('ElevenLabs not initialized or API key missing')));
          }
          ...
          if (success) {
            logger.debug('✅ Voice test completed successfully');
            return {
              success: true,
            ...
            throw new Error(new Error(new Error('Voice synthesis failed')));
          }
        } catch (error) {
