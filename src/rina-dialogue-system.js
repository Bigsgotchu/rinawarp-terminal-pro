class RinaDialogueSystem {
  constructor() {
    ...
    this.seaThemes = {
      puns: [
        'shell-ebrate', 'fin-tastic', 'o-fish-ally', 'sea-riously', 
        'wave-derful', 'mer-velous', 'tide-turning', 'current-ly',
        'sea-nior', 'depth-initely', 'shore-ly', 'whale-come'
      ],
      expressions: [
        'Holy mackerel!', 'Shiver me timbers!', 'Batten down the hatches!',
        'Anchors aweigh!', 'Full steam ahead!', 'Smooth sailing!',
        'All hands on deck!', 'Ship-shape!', 'Steady as she goes!'
      ],
      emojis: [
        '🌊', '🐠', '🐟', '🦈', '🐙', '🦑', '🐚', '🪸', '⚓', '🚢',
        '🏴‍☠️', '🧜‍♀️', '🌅', '🌙', '⭐', '✨', '💫', '🌟'
  ...
  updateSentiment(text) {
    const positiveWords = ['good', 'great', 'awesome', 'perfect', 'excellent', 'amazing', 'love', 'thanks'];
    const negativeWords = ['bad', 'terrible', 'awful', 'hate', 'stupid', 'annoying', 'frustrated', 'error'];
  ...
  generateResponse(context, baseResponse) {
    ...
    case 'mood':
      templates = this.dialogueTemplates.mood_responses[mood] || 
  ...
  enhanceResponse(response, mood, confidence) {
    ...
    if (mood === 'sassy') {
      const sassModifier = this.emotionalModifiers.sass[Math.floor(Math.random() * this.emotionalModifiers.sass.length)];
      enhanced = enhanced.replace(/\b(you|your)\b/gi, `$1, ${sassModifier}`);
    }
        
    // Add confidence boosters
    if (confidence > 0.8) {
      const confidentModifier = this.emotionalModifiers.confidence[Math.floor(Math.random() * this.emotionalModifiers.confidence.length)];
  ...
  getTimeBasedAdvice() {
    ...
    if (hour >= 22 || hour < 6) {
      return this.dialogueTemplates.time_specific.late_night[Math.floor(Math.random() * this.dialogueTemplates.time_specific.late_night.length)];
    }
        
    if (hour >= 14 && hour < 16) {
      return this.dialogueTemplates.time_specific.break_time[Math.floor(Math.random() * this.dialogueTemplates.time_specific.break_time.length)];
    }
        
    return this.dialogueTemplates.time_specific.productivity_hours[Math.floor(Math.random() * this.dialogueTemplates.time_specific.productivity_hours.length)];
  }
    
  getSessionStats() {
    const stats = this.sessionContext;
    const successRate = stats.commandsThisSession > 0 ? 
      ((stats.commandsThisSession - stats.errorCount) / stats.commandsThisSession * 100).toFixed(1) : 0;
  ...
  getContextualEmotionalResponse(command, outcome, userMood = 'neutral') {
    ...
    if (Math.random() > 0.8) {
      const quirk = this.loreCallbacks.personality_quirks[Math.floor(Math.random() * this.loreCallbacks.personality_quirks.length)];
      baseResponse += ` ${quirk}`;
    }
  ...
  detectEmotionalContext(command, outcome, userMood) {
    ...
    if (outcome === 'success' && (commandLower.includes('fix') || commandLower.includes('solve') || commandLower.includes('debug'))) {
      return 'confident';
    }
        
    // Check for uncertainty triggers
    if (outcome === 'error' && (commandLower.includes('complex') || commandLower.includes('difficult'))) {
      return 'uncertain';
    }
    ...
    if (commandLower.includes('build') || commandLower.includes('compile') || commandLower.includes('deploy')) {
      return 'dramatic';
    }
  ...
  getLoreCallback(command) {
    ...
    if (commandLower.includes('debug') || commandLower.includes('fix') || commandLower.includes('error')) {
      return this.loreCallbacks.technical_memories[Math.floor(Math.random() * this.loreCallbacks.technical_memories.length)];
    }
        
    // Adventures for exploration commands
    if (commandLower.includes('search') || commandLower.includes('find') || commandLower.includes('explore')) {
      return this.loreCallbacks.adventures[Math.floor(Math.random() * this.loreCallbacks.adventures.length)];
    }
        
    // Wisdom for complex operations
    if (commandLower.includes('build') || commandLower.includes('compile') || commandLower.includes('complex')) {
      return this.loreCallbacks.wisdom[Math.floor(Math.random() * this.loreCallbacks.wisdom.length)];
    }
        
    // Random callback for other commands
    if (Math.random() > 0.9) {
      const allCallbacks = [...this.loreCallbacks.technical_memories, ...this.loreCallbacks.adventures, ...this.loreCallbacks.wisdom];
  ...
  updateMoodEngine(userInput, commandSuccess, responseTime) {
    ...
    this.sessionContext.lastEmotionalState = moodModifiers[Math.floor(Math.random() * moodModifiers.length)];
