...
class RinaPersonalityEngine {
  ...
  assessComplexity(command) {
    ...
    const complexityScore = complexKeywords.filter(keyword => cmd.includes(keyword)).length + pipes + options;
  ...
  getSessionStats() {
    ...
    return {
      ...
      stats: {
        ...
        favoriteCommands: this.getFavoriteCommands(),
        sessionDuration: this.sessionMemory.length > 0 ? 
          Date.now() - new Date(this.sessionMemory[0].timestamp).getTime() : 0
      }
    };
  }
}
