...
export class VoiceAIIntegration {
    ...
    setupVoiceControls() {
        ...
        const voiceButton = document.querySelector('#voiceControlBtn') || 
...
Focus on being helpful and concise. If the command is unclear, ask for clarification.`;
    ...
    gatherContext() {
        return {
            ...
            system: navigator.platform.includes('Mac') ? 'macOS' : 
                    navigator.platform.includes('Win') ? 'Windows' : 'Unix-like'
        };
    }
    ...
    async handleAIResponse(response, originalCommand) {
        ...
        const responseText = voiceResponse || explanation || 
