...
class LLMAPIClient {
  ...
  async generateResponse(prompt, options = {}) {
    ...
    while (retries >= 0) {
      try {
        ...
        console.warn(`🔄 Retrying LLM request (${this.config.retries - retries}/${this.config.retries}):`, error.message);
    ...
    const response = await fetch(`${this.providers.openai.baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are Rina, an advanced AI assistant for terminal operations. Be helpful, concise, and provide actionable advice for command-line tasks.'
          },
          {
            role: 'user',
            content: prompt
  ...
  async callGoogle(prompt, options) {
    ...
    const response = await fetch(`${this.providers.google.baseUrl}/models/${model}:generateContent?key=${this.config.apiKey}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
