...
export class ProcessLifecycleManager {
  ...
  registerProcess(id, processInfo) {
    ...
    console.log(`[ProcessManager] Registered process: ${id}`);
  ...
  startHealthMonitoring(processId) {
    const healthCheck = setInterval(async () => {
      ...
      try {
        await this.performHealthCheck(process);
      } catch (error) {
        console.error(`[ProcessManager] Health check failed for ${processId}:`, error);
        await this.handleUnhealthyProcess(process);
      }
  ...
  async performHealthCheck(process) {
    ...
    if (typeof process.getMemoryUsage === 'function') {
      ...
      if (memUsage > this.config.memoryThreshold) {
        console.warn(`[ProcessManager] High memory usage detected: ${process.id} (${Math.round(memUsage / 1024 / 1024)}MB)`);
  ...
  async defaultRecoveryStrategy(process) {
    ...
    console.log(`[ProcessManager] Attempting restart ${process.restartCount + 1}/${this.config.maxRestarts} for ${process.id}`);
  ...
  async restartProcess(process) {
    ...
    console.log(`[ProcessManager] Successfully restarted process: ${process.id}`);
  ...
  setupRecoveryStrategy(processId) {
    const strategy = {
      recover: async (process) => {
        console.log(`[ProcessManager] Custom recovery for ${processId}`);
  ...
  async cleanupProcess(processId) {
    ...
    console.log(`[ProcessManager] Process cleanup complete: ${processId}`);
  }
  ...
  async cleanupAll() {
    ...
    const cleanupPromises = Array.from(this.processes.keys()).map(id => 
  ...
  emitHealthUpdate(process) {
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('process:health-update', {
  ...
  emitProcessRestart(process) {
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('process:restart', {
  ...
  emitResourceAlert(alert) {
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('process:resource-alert', {
        detail: alert
      }));
    }
  }
