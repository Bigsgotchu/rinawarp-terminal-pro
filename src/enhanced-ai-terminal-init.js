    ...
    try {
        ...
        if (initialized) {
            ...
            window.processAICommand = async function() {
                ...
                if (!input) {
                    responseDiv.innerHTML = '🧜‍♀️ <em>*flips tail impatiently*</em> Come on darling, give me something to work with!';
                    return;
                }
...
async function gatherTerminalContext() {
    ...
    try {
        if (window.terminalState.shellHarness) {
            const gitResult = await window.terminalState.shellHarness.execute('git status --porcelain', { silent: true });
            context.gitStatus = {
...
function displayEnhancedResponse(result, responseDiv) {
    ...
    if (result.confidence) {
        const confidenceColor = result.confidence > 0.8 ? '#00FF88' : result.confidence > 0.5 ? '#FFD93D' : '#FF6B6B';
    ...
    if (result.suggestions && result.suggestions.length > 0) {
        html += `<div style="color: #00FF88; margin-top: 10px;">
                    ...
                    ${result.suggestions.slice(0, 3).map(s => `• ${s}`).join('<br>')}
                 </div>`;
    }
...
async function offerCodeExecution(result) {
    ...
    const shouldExecute = confirm('🧜‍♀️ I generated some code! Would you like me to save it to a file or execute it?');
...
async function executeGeneratedCode(code, result) {
    try {
        ...
        if (code.includes('#!/bin/bash') || result.type === 'bash') {
            ...
            await window.terminalState.shellHarness.execute(`echo "${code.replace(/"/g, '\\"')}" > ${tempFile}`);
            command = `./${tempFile}`;
        }
...
window.processAICommand = window.processAICommand || function() {
    const input = document.getElementById('aiInput')?.value?.trim();
    const responseDiv = document.getElementById('aiResponse');
