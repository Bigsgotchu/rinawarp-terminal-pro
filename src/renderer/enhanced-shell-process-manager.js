...
export class EnhancedShellProcessManager extends ShellProcessManager {
  ...
  async performHealthCheck() {
    try {
      ...
      const memoryOk = this.healthMetrics.resourceUsage.memory < this.lifecycleOptions.maxMemoryUsage;
      ...
      if (isResponsive && memoryOk && cpuOk) {
        ...
        if (!memoryOk) this.log(`High memory usage: ${Math.round(this.healthMetrics.resourceUsage.memory / 1024 / 1024)}MB`, 'warning');
  ...
  handleResourceAlert(alert) {
    this.log(`Resource alert: ${alert.resourceType} usage ${Math.round(alert.value / 1024 / 1024)}MB exceeds threshold`, 'warning');
  ...
  getEnhancedStatus() {
    ...
    return {
      ...
      lifecycle: {
        ...
        avgResponseTime: this.healthMetrics.responseTime.length > 0 
          ? this.healthMetrics.responseTime.reduce((a, b) => a + b) / this.healthMetrics.responseTime.length 
          : 0
      }
    };
  }
