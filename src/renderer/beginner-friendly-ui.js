...
class BeginnerFriendlyUI {
  ...
  displayAssistantResponse(response) {
    ...
    // Add explanation safely
    const explanationText = document.createTextNode(response.explanation);
    responseDiv.appendChild(explanationText);

    // Add command suggestion if present
    if (response.command) {
      const commandDiv = document.createElement('div');
      commandDiv.className = 'command-suggestion';

      const strongElem = document.createElement('strong');
      strongElem.textContent = 'Command:';
      commandDiv.appendChild(strongElem);

      commandDiv.appendChild(document.createTextNode(' '));

      const codeElem = document.createElement('code');
      codeElem.textContent = response.command;
      commandDiv.appendChild(codeElem);

      const insertBtn = document.createElement('button');
      insertBtn.className = 'insert-btn';
      insertBtn.textContent = 'Insert';
      insertBtn.onclick = () => this.insertCommand(response.command);
      commandDiv.appendChild(insertBtn);

      responseDiv.appendChild(commandDiv);
    }
    ...
    if (response.example) {
      ...
      const strongElem = document.createElement('strong');
      strongElem.textContent = 'Example:';
      exampleDiv.appendChild(strongElem);

      exampleDiv.appendChild(document.createTextNode(' ' + response.example));
      responseDiv.appendChild(exampleDiv);
    }
  ...
  insertCommand(command) {
    ...
    try {
      if (this.terminalManager && this.terminalManager.insertCommand) {
        ...
        console.warn('Terminal manager or insertCommand method not available');
      }
    } catch (error) {
  ...
  setupAccessibilityFeatures() {
    ...
    decreaseBtn.setAttribute('onclick', 'beginnerUI.adjustFontSize(-1)');
    decreaseBtn.setAttribute('title', 'Decrease Font Size');
    decreaseBtn.textContent = 'A-';

    const increaseBtn = document.createElement('button');
    increaseBtn.setAttribute('onclick', 'beginnerUI.adjustFontSize(1)');
    increaseBtn.setAttribute('title', 'Increase Font Size');
    increaseBtn.textContent = 'A+';

    const resetBtn = document.createElement('button');
    resetBtn.setAttribute('onclick', 'beginnerUI.resetFontSize()');
    resetBtn.setAttribute('title', 'Reset Font Size');
  ...
  showTooltip(message, type = 'info') {
    ...
    tooltipIcon.textContent = type === 'warning' ? '⚠️' : '💡';
    ...
    tooltipContent.appendChild(messageText);
...
class GuidedTour {
  ...
  createTourOverlay() {
    ...
    tourTitle.textContent = this.tutorial.title;

    const tourProgress = document.createElement('div');
    tourProgress.className = 'tour-progress';

    const stepCounter = document.createElement('span');
    stepCounter.className = 'step-counter';
    stepCounter.innerHTML = `Step <span id="current-step">1</span> of ${this.tutorial.steps.length}`;
    ...
    progressFill.id = 'tour-progress';

    progressBar.appendChild(progressFill);
    tourProgress.appendChild(stepCounter);
    tourProgress.appendChild(progressBar);
    tourHeader.appendChild(tourTitle);
    tourHeader.appendChild(tourProgress);
    ...
    const stepCommand = document.createElement('div');
    stepCommand.id = 'step-command';
    stepCommand.className = 'step-command hidden';

    tourBody.appendChild(stepTitle);
    tourBody.appendChild(stepContent);
    tourBody.appendChild(stepCommand);

    // Tour footer
    const tourFooter = document.createElement('div');
    tourFooter.className = 'tour-footer';

    const prevBtn = document.createElement('button');
    prevBtn.id = 'tour-prev';
    prevBtn.setAttribute('onclick', 'currentTour.previousStep()');
    prevBtn.disabled = true;
    prevBtn.textContent = 'Previous';

    const skipBtn = document.createElement('button');
    skipBtn.id = 'tour-skip';
    skipBtn.setAttribute('onclick', 'currentTour.skip()');
    skipBtn.textContent = 'Skip Tour';

    const nextBtn = document.createElement('button');
    nextBtn.id = 'tour-next';
    nextBtn.setAttribute('onclick', 'currentTour.nextStep()');
    nextBtn.textContent = 'Next';

    tourFooter.appendChild(prevBtn);
    tourFooter.appendChild(skipBtn);
    tourFooter.appendChild(nextBtn);

    tourContent.appendChild(tourHeader);
    tourContent.appendChild(tourBody);
    tourContent.appendChild(tourFooter);
    this.overlay.appendChild(tourContent);
  ...
  showStep(stepIndex) {
    ...
    if (step.command) {
      // Create command HTML safely
      commandDiv.textContent = '';
      const commandContainer = document.createElement('div');
      commandContainer.className = 'command-to-type';

      const commandText = document.createElement('strong');
      commandText.textContent = 'Try this command:';

      const commandCode = document.createElement('code');
      commandCode.textContent = step.command;

      const insertButton = document.createElement('button');
      insertButton.setAttribute('onclick', `beginnerUI.insertCommand('${step.command}')`);
      insertButton.textContent = 'Insert';

      commandContainer.appendChild(commandText);
      commandContainer.appendChild(commandCode);
      commandContainer.appendChild(insertButton);

      commandDiv.appendChild(commandContainer);
...
class SmartHintSystem {
  ...
  showHint(id, message, duration = 3000) {
    ...
    hintContent.appendChild(closeButton);
  ...
  addCommandToBuilder(command) {
    ...
    // Create command text span safely
    const commandSpan = document.createElement('span');
    commandSpan.className = 'command-text';
    commandSpan.textContent = command; // Use textContent to prevent XSS
    ...
    // Append children to command element
    commandElement.appendChild(commandSpan);
  ...
  showCommandPreview(command) {
    ...
    description.textContent = 'This command will execute:';
    body.appendChild(description);

    const codeElement = document.createElement('code');
    codeElement.className = 'preview-command';
    codeElement.textContent = command; // Safe assignment
    ...
    explanationTitle.textContent = 'What this does:';
    explanationDiv.appendChild(explanationTitle);

    const explanationText = document.createElement('p');
    explanationText.textContent = this.explainCommand(command); // Safe assignment
...
class TaskWalkthrough {
  ...
  createModal() {
    ...
    // Create walkthrough content container
    const walkthroughContent = document.createElement('div');
    walkthroughContent.className = 'walkthrough-content';
    ...
    const title = document.createElement('h3');
    title.textContent = this.taskData.title; // Safe assignment
    header.appendChild(title);
    ...
    stepSpan.id = 'walkthrough-step';
    stepSpan.textContent = 'Step 1';
    stepIndicator.appendChild(stepSpan);

    stepIndicator.appendChild(document.createTextNode(' of ' + this.taskData.steps.length));
    ...
    description.id = 'walkthrough-description';
    body.appendChild(description);

    const command = document.createElement('div');
    command.id = 'walkthrough-command';
    command.className = 'walkthrough-command hidden';
    ...
    const prevBtn = document.createElement('button');
    prevBtn.id = 'walkthrough-prev';
    prevBtn.textContent = 'Previous';
    prevBtn.disabled = true;
    footer.appendChild(prevBtn);

    const skipBtn = document.createElement('button');
    skipBtn.id = 'walkthrough-skip';
    skipBtn.textContent = 'Skip';
    footer.appendChild(skipBtn);

    const nextBtn = document.createElement('button');
    nextBtn.id = 'walkthrough-next';
    nextBtn.textContent = 'Next';
    footer.appendChild(nextBtn);

    // Assemble the modal
    walkthroughContent.appendChild(header);
    walkthroughContent.appendChild(body);
    walkthroughContent.appendChild(footer);
    this.modal.appendChild(walkthroughContent);
  ...
  setupEventListeners() {
    this.modal
      ...
      .addEventListener('click', _e => this.previousStep());
  ...
  showStep(stepIndex) {
    ...
    if (step.command) {
      ...
      const commandContainer = document.createElement('div');
      commandContainer.className = 'command-to-execute';

      // Create label
      const strongLabel = document.createElement('strong');
      strongLabel.textContent = 'Execute this command:';
      commandContainer.appendChild(strongLabel);

      // Create code element
      const codeElement = document.createElement('code');
      codeElement.textContent = step.command; // Safe assignment
      ...
      executeButton.onclick = () => {
        this.ui.executeQuickAction(step.command);
