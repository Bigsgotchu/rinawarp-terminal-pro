...
export class SyntaxHighlighter {
  constructor(terminal) {
    ...
    this.fileExtensions = {
      ...
      code: ['.js', '.ts', '.jsx', '.tsx', '.py', '.java', '.cpp', '.c', '.cs', '.rb', '.go', '.rust', '.swift'],
  ...
  highlightPowerShell(text) {
    ...
    highlighted = highlighted.replace(this.patterns.powershell.comment, 
      match => colors.comment + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.powershell.string,
      match => colors.string + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.powershell.cmdlet,
      match => colors.cmdlet + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.powershell.parameter,
      match => colors.parameter + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.powershell.variable,
      match => colors.variable + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.powershell.number,
  ...
  highlightGit(text) {
    ...
    highlighted = highlighted.replace(this.patterns.git.command,
      match => colors.command + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.git.branch,
      match => colors.branch + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.git.hash,
      match => colors.hash + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.git.remote,
  ...
  highlightNpm(text) {
    ...
    highlighted = highlighted.replace(this.patterns.npm.command,
      match => colors.command + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.npm.package,
      match => colors.package + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.npm.version,
  ...
  highlightNpmOutput(text) {
    ...
    highlighted = highlighted.replace(/npm ERR!/g,
      colors.error + 'npm ERR!' + this.reset);
    
    highlighted = highlighted.replace(/npm WARN/g,
      colors.warning + 'npm WARN' + this.reset);
    
    highlighted = highlighted.replace(/added \d+ packages?/g,
      match => colors.success + match + this.reset);
    
    highlighted = highlighted.replace(/updated \d+ packages?/g,
      match => colors.success + match + this.reset);
    
    highlighted = highlighted.replace(/removed \d+ packages?/g,
  ...
  highlightDocker(text) {
    ...
    highlighted = highlighted.replace(this.patterns.docker.command,
      match => colors.command + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.docker.image,
      match => colors.image + match + this.reset);
    
    highlighted = highlighted.replace(this.patterns.docker.port,
  ...
  highlightDockerOutput(text) {
    ...
    highlighted = highlighted.replace(/Up \d+ \w+/g,
      match => colors.running + match + this.reset);
    
    highlighted = highlighted.replace(/Exited \(\d+\)/g,
  ...
  highlightGeneric(text) {
    ...
    // Highlight errors
    highlighted = highlighted.replace(/\b(error|fail|failed|exception)\b/gi,
      match => '\x1b[31m' + match + this.reset);
    
    // Highlight warnings
    highlighted = highlighted.replace(/\b(warning|warn)\b/gi,
      match => '\x1b[33m' + match + this.reset);
    
    // Highlight success
    highlighted = highlighted.replace(/\b(success|succeeded|complete|completed|done)\b/gi,
      match => '\x1b[32m' + match + this.reset);
    
    // Highlight numbers
    highlighted = highlighted.replace(/\b\d+\.?\d*\b/g,
      match => '\x1b[36m' + match + this.reset);
