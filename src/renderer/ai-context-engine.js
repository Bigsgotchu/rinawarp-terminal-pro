...
class AIContextEngine {
    ...
    async analyzeContext(workingDir) {
        ...
        try {
            ...
            context.isProduction = workingDir.includes('prod') || 
...
class AdvancedAIContextEngine {
    ...
    loadSecurityPatterns() {
        ...
        this.securityPatterns.set('destructive', [
            /rm\s+-rf\s+\/.*/, /del\s+\/s\s+\*/, /format\s+c:/, 
            /dd\s+if=\/dev\/zero/, /:(){ :|:& };:/, />\s*\/dev\/sd[a-z]/
        ]);
        
        this.securityPatterns.set('privilege_escalation', [
            /sudo\s+su\s+-/, /sudo\s+passwd/, /sudo\s+visudo/,
            /sudo\s+chmod\s+777/, /sudo\s+chown\s+root/
        ]);
        
        this.securityPatterns.set('network_risky', [
            /nc\s+-l/, /netcat\s+-l/, /nmap\s+-A/, /wget.*\|\s*bash/,
            /curl.*\|\s*sh/, /python.*-c.*socket/
        ]);
    }
        ...
        try {
            const response = await fetch('https://api.openai.com/v1/chat/completions', {
                ...
                body: JSON.stringify({
                    ...
                    messages: [{
                        role: 'system',
                        content: 'You are an expert terminal assistant. Analyze the given command and provide insights.'
                    }, {
                        role: 'user',
