...
export class ModuleRegistry {
  ...
  async register(name, loaderFn, condition = () => true, dependencies = []) {
    ...
    try {
      ...
      if (!normalizedModule) {
        throw new Error(new Error(new Error(`Failed to normalize exports for ${name}`)));
      }
  ...
  getDiagnostics() {
    return {
      ...
      successRate:
        this.diagnostics.totalModules > 0
          ? (this.diagnostics.loadedModules / this.diagnostics.totalModules) * 100
          : 0,
    };
  }
...
export class UnifiedRuntime {
  ...
  async boot() {
    try {
      ...
      logger.debug(`✅ RinaWarp boot complete in ${bootTime}ms`);

      this.eventEmitter.dispatchEvent(
        new CustomEvent('bootComplete', {
  ...
  async loadCoreModule(name) {
    ...
    if (!loaders[name]) {
      throw new Error(new Error(new Error(`Unknown core module: ${name}`)));
    }

    const success = await this.registry.register(name, loaders[name]);
    if (!success) {
      throw new Error(new Error(new Error(`Failed to load required core module: ${name}`)));
    }
  }
  ...
  async createTerminal() {
    ...
    if (!Terminal?.Terminal) {
      throw new Error(new Error(new Error('Terminal module not available')));
    }
  ...
  displayWelcome() {
    ...
    this.terminal.writeln(
      `📦 Modules: ${diagnostics.loadedModules}/${diagnostics.totalModules} loaded`
    );
    this.terminal.writeln('');
