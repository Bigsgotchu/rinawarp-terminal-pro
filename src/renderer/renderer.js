...
async function loadAdvancedFeatures() {
    try {
        ...
        AdvancedAIContextEngine = class { constructor() { console.log('AI Context Engine - Fallback Mode'); } };
        PerformanceMonitoringDashboard = class { constructor() { console.log('Performance Monitor - Fallback Mode'); } };
...
class CommandHistoryManager {
    constructor() {
        ...
        this.commonCommands = [
            ...
            'find', 'which', 'ps', 'kill', 'top', 'chmod', 'chown', 'tar', 'zip',
...
class AdvancedGitPlugin {
    ...
    addGitStatusBar(api) {
        ...
        const gitControls = api.addStatusBarItem('git-controls', `
    ...
    async handlePushWorkflow(terminalData, command) {
        const api = window.terminalManager?.pluginAPI;
        if (api) {
    ...
    async handleMergeWorkflow(terminalData, command) {
        const api = window.terminalManager?.pluginAPI;
        if (api) {
    ...
    async getStatusInfo(execAsync, directory) {
        try {
            ...
            const { stdout: ahead } = await execAsync('git rev-list --count @{u}..HEAD', { cwd: directory }).catch(() => ({ stdout: '0' }));
            const { stdout: behind } = await execAsync('git rev-list --count HEAD..@{u}', { cwd: directory }).catch(() => ({ stdout: '0' }));
...
class AIAssistantPlugin {
    ...
    async onCommandSuggestion(input) {
        ...
        try {
            if (window.terminalManager && window.terminalManager.settings.realAI && window.terminalManager.settings.aiApiKey) {
        ...
        if (input.includes('git')) {
            suggestions.push('git status', 'git add .', 'git commit -m ""', 'git push', 'git pull', 'git branch', 'git checkout', 'git merge');
        } else if (input.includes('npm')) {
            suggestions.push('npm install', 'npm start', 'npm run build', 'npm test', 'npm run dev', 'npm update', 'npm audit');
        } else if (input.includes('docker')) {
            suggestions.push('docker ps', 'docker images', 'docker build .', 'docker run', 'docker exec', 'docker logs', 'docker stop');
        } else if (input.includes('kubectl')) {
            suggestions.push('kubectl get pods', 'kubectl get services', 'kubectl apply -f', 'kubectl describe', 'kubectl logs');
        }
    ...
    async getOpenAISuggestions(input, context, apiKey) {
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [{
                    role: 'system',
                    content: `You are a terminal command assistant. Given a partial command and context, suggest 3-5 most relevant complete commands. Return only the commands, one per line, no explanations.`
                }, {
                    role: 'user',
...
class SessionManager {
    ...
    restoreSession(sessionId) {
        ...
        manager.terminals.forEach((_, terminalId) => {
            if (terminalId !== 1) { // Keep at least one terminal
...
class GitHubSyncProvider {
    ...
    async upload(data) {
        ...
        let sha = null;
        try {
            const response = await fetch(`https://api.github.com/repos/${this.repo}/contents/${this.filename}?ref=${this.branch}`, {
                headers: {
                    'Authorization': `Bearer ${this.token}`,
                    'Accept': 'application/vnd.github.v3+json'
                }
...
class PluginAPI {
    ...
    matchesShortcut(event, shortcut) {
        ...
        let hasCtrl = keys.includes('ctrl') ? event.ctrlKey : !event.ctrlKey;
        let hasShift = keys.includes('shift') ? event.shiftKey : !event.shiftKey;
        let hasAlt = keys.includes('alt') ? event.altKey : !event.altKey;
...
class TerminalManager {
        ...
        try {
            ...
            if (featuresLoaded) {
                ...
                if (PerformanceMonitoringDashboard) {
                    ...
                    this.pluginAPI.addStatusBarItem('performance-dashboard', `
                ...
                if (WorkflowAutomationEngine) {
                    ...
                    this.pluginAPI.addStatusBarItem('workflow-automation', `
                ...
                if (EnhancedSecurityEngine) {
                    ...
                    this.pluginAPI.addStatusBarItem('security-dashboard', `
                ...
                if (NextGenUIEngine) {
                    ...
                    this.pluginAPI.addStatusBarItem('next-gen-ui', `
            ...
            if (this.aiEngine) {
                this.pluginAPI.addStatusBarItem('ai-features', `
    ...
    showAIFeaturesModal() {
        const aiModal = this.pluginAPI.createModal('🤖 Advanced AI Features', `
    ...
    showWorkflowManagerModal() {
        const workflowModal = this.pluginAPI.createModal('⚡ Workflow Automation Manager', `
    ...
    showAdvancedUIModal() {
        const uiModal = this.pluginAPI.createModal('🎨 Next-Gen UI Features', `
    ...
    addQuickAccessButtons() {
        ...
        this.pluginAPI.addStatusBarItem('session-manager', `
        ...
        this.pluginAPI.addStatusBarItem('cloud-sync', `
        ...
        this.pluginAPI.addStatusBarItem('find-terminal', `
        ...
        document.addEventListener('keydown', async (e) => {
            ...
            if (e.key === 'Enter') {
                ...
                if (buffer.trim()) {
                    ...
                    await this.pluginManager.executeHook('terminal-created', { terminalId: this.activeTerminalId });
                }
                this.commandBuffers.set(this.activeTerminalId, '');
                this.hideSuggestions();
    ...
    showSuggestions(suggestions) {
        ...
        const activeTerminal = document.querySelector(`.terminal-pane[data-tab-id="${this.activeTerminalId}"]`);
        if (activeTerminal) {
    ...
    splitTerminal(direction) {
        const activePane = document.querySelector(`.terminal-pane[data-tab-id="${this.activeTerminalId}"]`);
    ...
    showContextMenu(x, y) {
        ...
        const defaultItems = [
            ...
            { id: 'close-tab', label: 'Close Tab', handler: () => this.closeTab(this.activeTerminalId), icon: '❌' },
            { id: 'separator2', label: '---', handler: null },
            { id: 'split-h', label: 'Split Horizontally', handler: () => this.splitTerminal('horizontal'), icon: '⬌' },
            { id: 'split-v', label: 'Split Vertically', handler: () => this.splitTerminal('vertical'), icon: '⬍' },
    ...
    switchToNextTab() {
        ...
        const currentIndex = tabs.findIndex(tab => parseInt(tab.dataset.tabId) === this.activeTerminalId);
    ...
    showSessionManager() {
        const sessionModal = this.pluginAPI.createModal('Session Manager', `
    ...
    showCloudSyncManager() {
        const syncModal = this.pluginAPI.createModal('Cloud Sync Manager', `
...
class PluginDevelopmentAPI {
    ...
    setupPluginAPI() {
        ...
        window.WarpPluginAPI = {
            ...
            getActiveTerminal: () => this.terminalManager.terminals.get(this.terminalManager.activeTerminalId),
            ...
            // UI manipulation
            showNotification: (message, type = 'info') => this.terminalManager.pluginAPI.showNotification(message, type),
    ...
    setSetting(key, value) {
        this.terminalManager.settings[key] = value;
        this.terminalManager.saveSettings();
        this.eventBus.dispatchEvent(new CustomEvent('setting-changed', { 
            detail: { key, value } 
        }));
    }
