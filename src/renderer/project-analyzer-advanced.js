...
class ProjectAnalyzer {
  ...
  async analyzeDockerfile(analysis) {
    const dockerFiles = analysis.files.filter(f => 
      f.toLowerCase().includes('dockerfile') || f === 'Dockerfile'
    );
  ...
  async analyzeProjectStructure(analysis) {
    ...
    structure.hasTests = analysis.files.some(f => 
      f.includes('test') || f.includes('spec') || f.includes('__tests__')
    );
    
    structure.hasDocs = analysis.files.some(f => 
      f.includes('doc') || f.includes('README') || f.includes('.md')
    );
    
    structure.hasConfig = analysis.configFiles.size > 0;
    
    structure.hasSrc = analysis.files.some(f => 
      f.startsWith('src/') || f.startsWith('lib/') || f.startsWith('app/')
    );
    
    structure.hasAssets = analysis.files.some(f => 
  ...
  determineProjectType(analysis) {
    ...
    if (analysis.files.includes('pyproject.toml') || analysis.files.includes('setup.py')) return 'python';
    ...
    if (analysis.languages.size > 0) {
      const primaryLang = Array.from(analysis.languages.entries())
