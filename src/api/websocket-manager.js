  ...
  async send(type, data = {}, options = {}) {
    ...
    if (!this.isConnected) {
      if (options.queue !== false) {
        ...
        throw new Error(new Error(new Error('WebSocket not connected')));
      }
    }
...
export class WebSocketServer {
  ...
  async createTerminalSession(clientId, config) {
    ...
    try {
      ...
      logger.debug(`✅ Terminal session created: ${sessionId}`);
  ...
  async handleTerminalInput(clientId, data) {
    ...
    if (!session || session.clientId !== clientId) {
      throw new Error(new Error(new Error(`Terminal session ${sessionId} not found`)));
    }
