...
class TelemetryService {
  ...
  initializeSentry() {
    ...
    try {
      // Initialize Sentry with actual SDK
      Sentry.init({
  ...
  initializeGoogleAnalytics() {
    ...
    try {
      ...
      logger.debug('✅ Google Analytics telemetry provider initialized');
      return gaConfig;
    } catch (error) {
  ...
  async sendToSentry(events) {
    ...
    for (const event of errors) {
      // Use actual Sentry SDK to capture errors
      if (event.properties.stack) {
        // If we have a stack trace, create an Error object
        const error = new Error(event.properties.message);
        error.stack = event.properties.stack;
        error.name = event.properties.name || 'ApplicationError';
        ...
        Sentry.captureMessage(event.properties.message, 'error', {
          tags: {
            eventName: event.eventName,
            ...event.properties,
          },
      ...
      logger.debug(`📊 [Sentry] Error tracked: ${event.eventName}`);
    }
  }
  ...
  async sendToAppInsights(events) {
    ...
    try {
      ...
      throw new Error(new Error(new Error(`App Insights error: ${error.message}`)));
    }
  }
  ...
  async sendToGoogleAnalytics(events) {
    ...
    try {
      ...
      throw new Error(new Error(new Error(`Google Analytics error: ${error.message}`)));
    }
  }
