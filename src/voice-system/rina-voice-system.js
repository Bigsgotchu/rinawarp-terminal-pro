...
export class RinaVoiceSystem {
  ...
  async init() {
    ...
    logger.debug('✅ Rina Voice System initialized successfully');
  }
  ...
  async preloadCriticalAudio() {
    ...
    logger.debug('✅ Critical audio clips preloaded');
  }
  ...
  async preloadClip(clipKey, mood = 'neutral') {
    ...
    try {
      ...
      logger.debug(`⚠️ Error preloading ${clipKey} (using fallback):`, error.message);
      return false;
    }
  }
  ...
  async preloadFrequentPhrases() {
    ...
    logger.debug('✅ Frequent phrases preloaded');
  }
  ...
  async onModuleError(moduleName) {
    await this.speak('moduleError', {
      mood: 'technical',
      onComplete: () => {
        logger.debug(`📊 Module error reported for: ${moduleName}`);
      },
    });
  }
  ...
  async speak(keyOrText, options = {}) {
    ...
    // Check if this is a key from our voice map or direct text
    if (this.voiceMap.has(keyOrText)) {
      // This is a voice key - use the original speak logic
      return await this.speakVoiceKey(keyOrText, options);
    } else {
      // This is direct text - use text-to-speech
      return await this.speakDirectText(keyOrText, options);
    }
  }

  // Speak using a voice key (original logic)
  async speakVoiceKey(key, options = {}) {
    const {
      mood = this.currentMood,
      volume = this.config.baseVolume,
      onComplete = null,
  ...
  // Speak direct text
  async speakDirectText(text, options = {}) {
    if (!text) {
    ...
    if (this.config.enableSynthesis && this.fallbackSynthesis) {
      try {
        ...
        // Set up event listeners
        utterance.onend = () => {
          logger.debug('✅ Text synthesis completed');
          if (onComplete) onComplete();
        };
