...
export class RinaVoiceIntegration {
  ...
  setupEventListeners() {
    ...
    if (this.voiceEngine && this.voiceEngine.detectMood) {
  ...
  async handleVoiceCommand(command, _transcript, _mood) {
  ...
  async speak(text, options = {}) {
    try {
      if (this.isRinaEnabled && this.currentMode !== 'system') {
        if (this.currentMode === 'hybrid') {
          // In hybrid mode, use Rina for personality responses and system for commands
          if (options.isCommand) {
            return this.voiceEngine.speak(text, options);
          } else if (options.isPersonality || !options.isCommand) {
            return this.rinaVoice.speak(text, options);
          }
        }
        // For rina and elevenlabs modes, use Rina voice
        return this.rinaVoice.speak(text, options);
      } else {
        // System mode or Rina disabled
        return this.voiceEngine.speak(text, options);
      }
    } catch (error) {
