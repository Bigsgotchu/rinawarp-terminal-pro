...
export class EnhancedVoiceEngine {
  ...
  async init() {
    ...
    logger.debug('✅ Enhanced Voice Recognition Engine initialized');
  }
  ...
  setupContextEngine() {
    ...
    this.contextEngine = {
      currentDirectory: typeof process !== 'undefined' ? process.cwd() : '/',
      recentCommands: [],
      projectType: null,
      commandContext: null,
    };
  ...
  getCustomCommands() {
    return Array.from(this.customCommands.entries()).map(([voice, command]) => ({
      voice,
      command,
    }));
  }

  recordCommand(isCorrect) {
    this.accuracyMetrics.totalCommands++;
    if (isCorrect) {
      this.accuracyMetrics.correctCommands++;
    }
  ...
  toggleNoiseFilter() {
    if (this.noiseFilter) {
      this.noiseFilter.enabled = !this.noiseFilter.enabled;
    }
  }
