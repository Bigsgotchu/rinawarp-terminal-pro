  ...
  static async load(modulePath, options = {}) {
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        lastError = error;
        console.warn(`Failed to load module ${modulePath} (attempt ${i + 1}/${retries}):`, error.message);
...
export const initializeModule = async (modulePath, config = {}) => {
  try {
    ...
    throw new ModuleError(
      `Failed to initialize module: ${modulePath}`,
      modulePath,
      error
    );
  }
...
export const loadModules = async (modules, options = {}) => {
  ...
  if (parallel) {
    ...
    for (const module of modules) {
      try {
        const loaded = typeof module === 'string'
