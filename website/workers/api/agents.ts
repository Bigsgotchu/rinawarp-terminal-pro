...
const defaultAgents: Record<string, AgentPackage> = {
  ...
  'security-audit': {
    ...
    commands: [
      ...
      {
        name: 'secrets',
        steps: ["grep -r -E '(api_key|password|secret|token)' --include='*.env' . || echo 'No secrets found'"],
      },
  ...
  'docker-repair': {
    name: 'docker-repair',
    description:
      'Fix Docker environments automatically. Diagnoses and repairs common Docker issues like port conflicts, stuck containers, and network problems.',
    author: 'RinaWarp',
    version: '1.0.0',
  ...
  'npm-audit': {
    name: 'npm-audit',
    description:
      'Fix npm security vulnerabilities. Automatically updates and fixes security issues in your node_modules.',
    author: 'RinaWarp',
    version: '1.0.0',
