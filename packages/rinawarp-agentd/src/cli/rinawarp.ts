...
async function apiRequest(args: { method: 'GET' | 'POST' | 'PUT'; path: string; body?: unknown; useAuth?: boolean }) {
...
async function cmdTeamInviteCreate() {
  ...
  if (!email || !workspaceId) {
    console.error(
      'Usage: rinawarp team invite create --email <email> [--role member|admin|owner] [--workspace-id ws_...]'
    )
    process.exit(1)
  }
  const session = loadSession()
