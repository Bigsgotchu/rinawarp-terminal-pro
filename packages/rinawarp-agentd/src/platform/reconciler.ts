...
export function configureReconciler(args: {
  ...
  if (Number.isFinite(args.runtime_queue_stuck_after_sec))
    state.runtime_queue_stuck_after_sec = Math.max(60, Number(args.runtime_queue_stuck_after_sec))
  if (Number.isFinite(args.runtime_running_stuck_grace_sec))
    state.runtime_running_stuck_grace_sec = Math.max(30, Number(args.runtime_running_stuck_grace_sec))
  if (typeof args.runtime_auto_remediate === 'boolean') state.runtime_auto_remediate = args.runtime_auto_remediate
  if (Number.isFinite(args.max_runtime_remediations))
    state.max_runtime_remediations = Math.max(1, Number(args.max_runtime_remediations))
  if (Number.isFinite(args.archive_interval_sec))
    state.archive_interval_sec = Math.max(60, Number(args.archive_interval_sec))
  saveState(state)
  return state
}
