...
async function discoverRegionProbes(region: RegionId): Promise<ProbeDef[]> {
  ...
  for (const rule of rules) {
    try {
      ...
      const res = await fetch(`${apiServer}/api/v1/namespaces/${encodeURIComponent(rule.namespace)}/services${query}`, {
...
function stabilizedStatus(args: {
  ...
  if (args.observed === 'down' && runtime.consecutive_failures >= state.policy.consecutive_failures_for_down)
    return 'down'
...
export async function runHealthProbes(force = false): Promise<{
  ...
  regions?: Record<
    RegionId,
    {
      ...
      static_probes: number
      discovered_probes: number
    }
  >
