 ...
 */
const INFRASTRUCTURE_PATTERNS: Record<
...
> = {
  'aws-ecs': {
    ...
    scale: ['aws ecs update-service --cluster $CLUSTER --service $SERVICE_NAME --desired-count $COUNT'],
    logs: ['aws logs tail /ecs/$SERVICE_NAME --follow'],
  ...
  kubernetes: {
    deploy: ['kubectl apply -f deployment.yaml', 'kubectl rollout status deployment/$SERVICE_NAME'],
    scale: ['kubectl scale deployment/$SERVICE_NAME --replicas=$COUNT'],
    logs: ['kubectl logs -f deployment/$SERVICE_NAME'],
    status: ['kubectl get pods,svc,deployments'],
  },
  'docker-compose': {
    deploy: ['docker-compose up -d --build'],
    scale: ['docker-compose up -d --scale $SERVICE_NAME=$COUNT'],
  ...
  vercel: {
    ...
    logs: ['vercel logs $SERVICE_NAME'],
    status: ['vercel inspect $SERVICE_NAME'],
  },
  heroku: {
    deploy: ['git push heroku main'],
    scale: ['heroku ps:scale web=$COUNT'],
    logs: ['heroku logs --tail'],
    status: ['heroku ps'],
  },
  // Placeholder for other targets (legacy/unsupported)
  'aws-ec2': {
    deploy: ['# EC2 requires manual setup'],
    scale: ['# Auto-scale group'],
 ...
 */
async function generateLLMInfraPlan(config: LLMConfig, request: InfrastructureRequest): Promise<InfrastructurePlan> {
