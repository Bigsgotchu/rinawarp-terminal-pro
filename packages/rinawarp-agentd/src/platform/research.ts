...
function readState(): ResearchConfig {
  ...
  try {
    ...
    return {
      ...
      timeout_ms: Number.isFinite(parsed.timeout_ms) ? Math.max(500, Math.min(60_000, Number(parsed.timeout_ms))) : 8_000,
      max_bytes: Number.isFinite(parsed.max_bytes) ? Math.max(1_024, Math.min(1_000_000, Number(parsed.max_bytes))) : 100_000,
...
function stripHtml(body: string): string {
  return body.replace(/<script[\s\S]*?<\/script>/gi, " ").replace(/<style[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ");
}
...
export function configureResearch(input: {
  ...
  const next: ResearchConfig = {
    ...
    ...(Number.isFinite(input.timeout_ms) ? { timeout_ms: Math.max(500, Math.min(60_000, Number(input.timeout_ms))) } : {}),
    ...(Number.isFinite(input.max_bytes) ? { max_bytes: Math.max(1_024, Math.min(1_000_000, Number(input.max_bytes))) } : {}),
...
export async function runResearchFetch(input: {
  url: string;
}): Promise<{
  ok: boolean;
  error?: string;
