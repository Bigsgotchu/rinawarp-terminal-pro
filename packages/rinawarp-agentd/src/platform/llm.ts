...
function parseFixPlan(response: string): FixPlan {
  ...
  return {
    ...
    risk: parsed.risk === 'high' || parsed.risk === 'medium' || parsed.risk === 'low' ? parsed.risk : 'low',
 ...
 * Call OpenAI API
 */
async function callOpenAI(config: LLMConfig, messages: Array<{ role: string; content: string }>): Promise<string> {
  const model = config.model || 'gpt-4o'
  const url = config.baseURL ? `${config.baseURL}/v1/chat/completions` : 'https://api.openai.com/v1/chat/completions'
 ...
 * Call Anthropic API
 */
async function callAnthropic(config: LLMConfig, messages: Array<{ role: string; content: string }>): Promise<string> {
