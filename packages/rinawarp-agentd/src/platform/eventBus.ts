...
export async function initEventBus(): Promise<void> {
  ...
  if (!servers) return;
  const mode = String(process.env.RINAWARP_NATS_MODE || "core").trim().toLowerCase();
  try {
    ...
    if (mode === "jetstream") {
      ...
      const maxBytes = Math.max(1024 * 1024, Number(process.env.RINAWARP_JETSTREAM_MAX_BYTES || 50 * 1024 * 1024 * 1024));
      try {
