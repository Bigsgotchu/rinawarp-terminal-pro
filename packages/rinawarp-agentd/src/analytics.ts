...
function saveMetrics(metrics: PersistentMetrics): void {
  ...
  try {
    ...
    fs.writeFileSync(path.join(METRICS_DIR, 'metrics.json'), JSON.stringify(metrics, null, 2), 'utf8')
  } catch (error) {
...
export function getMetrics(): PersistentMetrics & {
  ...
  const mttrUnblockMs =
    metrics.unblock_runs === 0 ? 0 : Math.round(metrics.unblock_duration_ms_total / metrics.unblock_runs)
