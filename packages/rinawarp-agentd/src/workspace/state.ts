...
function appendEvent(db: WorkspaceDb, workspaceId: string, type: string, payload: Record<string, unknown>): WorkspaceEvent {
...
export function createInvite(args: {
  ...
  const expiresAt = new Date(Date.now() + Math.max(1, Math.min(24 * 14, args.expiresInHours)) * 60 * 60 * 1000).toISOString();
  db.invites[inviteId] = {
    invite_id: inviteId,
  ...
  appendEvent(db, ws.id, "invite_created", { invite_id: inviteId, email: args.email, role: args.role, send_email: args.sendEmail });
  appendAudit({
    workspace_id: ws.id,
    type: "invite_created",
    actor_id: args.actorId,
...
function recordInviteFailure(db: WorkspaceDb, tokenHash: string): { locked: boolean; retry_after_sec?: number } {
  ...
  return { locked: !!rec.locked_until_ms && rec.locked_until_ms > now, retry_after_sec: rec.locked_until_ms ? Math.ceil((rec.locked_until_ms - now) / 1000) : undefined };
}
...
export function updateInviteSecurityConfig(args: {
  ...
  db.invite_security = {
    ...
    ...(Number.isFinite(args.rate_limit_per_min) ? { rate_limit_per_min: Math.max(1, Number(args.rate_limit_per_min)) } : {}),
...
export function syncPush(args: {
  ...
  actorId: string;
}): { ok: true; new_version: number; conflicts: [] } | { ok: false; error: "version_conflict"; server_version: number } | null {
  const db = loadDb();
  const ws = db.workspaces[args.workspaceId];
  if (!ws) return null;
  if (mutationBlocked(ws)) {
...
export function getWorkspaceActorRole(args: { workspaceId: string; actorId: string; actorEmail: string }): WorkspaceRole | null {
  const db = loadDb();
  const ws = db.workspaces[args.workspaceId];
  if (!ws) return null;
...
export function applyStripeWebhookEvent(args: {
  ...
  appendAudit({
    ...
    metadata: { type: t, billing_status: ws.billing_status, billing_locked: ws.billing_locked, seats_allowed: ws.seats_allowed },
  });
  saveDb(db);
  return toSummary(ws);
}
