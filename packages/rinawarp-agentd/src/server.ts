...
import type {
  ...
  ExecutionReport,
  LicenseTier,
  PlanStep as CorePlanStep,
  ToolEvent,
} from '@rinawarp/core/enforcement/types.js'
...
import {
  enforceInviteAcceptCooldown,
  enforceInviteCreateRate,
  recordInviteAcceptFailure,
} from './workspace/securityStore.js'
import { appendSoc2Event } from './platform/soc2Log.js'
import {
  assignWorkspaceRegion,
  failoverDefaultRegion,
  getRegionHealth,
  getRegionMap,
  getWorkspaceRegion,
  setRegionHealth,
} from './platform/regions.js'
...
import {
  ...
  listRemoteRuns,
  resumeRemoteRun,
} from './platform/remoteRuns.js'
import {
  createWorkspaceObject,
  getWorkspaceObject,
  listWorkspaceObjects,
  updateWorkspaceObject,
} from './platform/workspaceObjects.js'
import {
...
} from './platform/workflowTemplates.js'
...
import {
  activeActiveWrite,
  configureActiveActive,
  getActiveActiveState,
  replayWorkspaceEvents,
  runReplicationDrill,
} from './platform/activeActive.js'
import { configureReconciler, getReconcilerState, runFullReconcile } from './platform/reconciler.js'
import {
  configureTokenLifecycle,
  getTokenLifecycleStatus,
  registerRefreshSession,
  revokeRefreshSession,
  rotateRefreshSession,
  validateRefreshSession,
} from './workspace/tokenLifecycle.js'
import {
  configureSecurityControls,
  getSecurityControlsState,
  runControlEvidenceDrill,
} from './platform/securityControls.js'
...
function validatePlanStep(step: PlanStep, index: number): string[] {
  ...
  if (step.requires_confirmation !== expected.requires_confirmation) {
    errors.push(
      `${prefix}.requires_confirmation must be ${String(expected.requires_confirmation)} for risk=${step.risk}`
    )
  }
...
function webhookRateLimitPerWindow(): number {
  const raw = Number(
    process.env.RINAWARP_WEBHOOK_RATE_LIMIT_PER_WINDOW || process.env.RINAWARP_WEBHOOK_RATE_LIMIT_PER_MIN || 120
  )
...
function actorFromRequest(req: http.IncomingMessage): { actorId: string; actorEmail: string } {
  ...
  const actorEmail =
    String(req.headers['x-rina-actor-email'] || 'owner@local')
      .trim()
      .toLowerCase() || 'owner@local'
...
function accountPlanFromEnv() {
  ...
  const renewsAt = String(
    process.env.RINAWARP_ACCOUNT_RENEWS_AT || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString()
  )
  return {
    plan,
    status,
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/v1/auth/revoke') {
        ...
        const out = revokeRefreshSession({
          jti: claims.jti,
          reason: String(body?.reason || 'manual_revoke').trim() || 'manual_revoke',
        })
        return sendJson(res, out.ok ? 200 : 404, {
          ok: out.ok,
          ...(out.ok ? {} : { error: 'refresh_session_not_found' }),
        })
      }
      ...
      if (req.method === 'POST' && url.pathname === '/v1/checkout/sessions') {
        ...
        if (!priceId) {
          return sendJson(res, 400, { ok: false, error: 'priceId is required' })
        }

        // Dynamically import Stripe (optional dependency)
        let stripe: any = null
        try {
          // eslint-disable-next-line @typescript-eslint/no-var-requires
          const stripeModule = require('stripe')
          const stripeKey = process.env.STRIPE_SECRET_KEY
          if (!stripeKey) {
            return sendJson(res, 503, { ok: false, error: 'stripe_not_configured' })
          }
          stripe = new stripeModule(stripeKey)
        } catch (importError) {
          console.error('[Checkout] Stripe import failed:', importError)
          return sendJson(res, 503, { ok: false, error: 'stripe_unavailable' })
        }
        ...
        try {
          ...
          if (workspaceId) {
            sessionParams.metadata = { workspace_id: workspaceId }
          }
          ...
          return sendJson(res, 500, { ok: false, error: stripeError.message })
        }
      }
      ...
      if (req.method === 'POST' && url.pathname === '/v1/licenses/generate') {
        ...
        // For now, return the generated key (in production, store in DB)
        const license = {
          key,
          tier: tier,
          customer_id: customerId,
      ...
      if (req.method === 'POST' && url.pathname === '/v1/feedback') {
        const body = (await readJson(req)) as { message?: string; email?: string; category?: string } | null
        ...
        console.log(`[Feedback] Received from ${email || 'anonymous'}: ${message.substring(0, 100)}...`)

        // Track in analytics
        trackServerEvent('api_feedback_submitted' as any, { category, has_email: !!email })

        return sendJson(res, 200, { ok: true, message: 'Feedback received' })
      }
      ...
      if (req.method === 'PUT' && url.pathname === '/v1/platform/health-probes/config') {
        const body = (await readJson(req)) as {
          ...
          probes?: Partial<
            Record<
              'us-east-1' | 'eu-west-1',
              Array<string | { url: string; class?: 'app' | 'db' | 'queue' | 'control-plane'; weight?: number }>
            >
          >
          policy?: {
      ...
      // --- AI streaming endpoint (Server-Sent Events for token streaming)
      if (req.method === 'GET' && url.pathname === '/v1/ai/stream') {
        const prompt = url.searchParams.get('prompt')
        if (!prompt) return sendJson(res, 400, { ok: false, error: 'prompt is required' })

        const apiKey = process.env.OPENAI_API_KEY
        if (!apiKey) return sendJson(res, 503, { ok: false, error: 'OpenAI API key not configured' })
        ...
        try {
          ...
          for await (const event of stream) {
            ...
            if (event.type === 'response.output_text.delta') {
              sseSend(res, 'message', { delta: event.delta })
            }
          }
      ...
      if (req.method === 'POST' && url.pathname === '/v1/plan') {
        ...
        if (validationErrors.length > 0) {
          return sendJson(res, 500, {
            ok: false,
            error: `planner produced invalid safety contract: ${validationErrors.join('; ')}`,
          })
        }
      ...
      if (req.method === 'POST' && url.pathname === '/v1/execute-plan') {
        ...
        if (validationErrors.length > 0) {
          return sendJson(res, 400, {
            ok: false,
            error: `invalid plan safety contract: ${validationErrors.join('; ')}`,
          })
        }
        const projectRoot = normalizeProjectRoot(body.projectRoot)
        ...
        if (cancelled) {
          ...
          trackServerEvent('plan_execution_complete', {
            planRunId,
            durationMs: runDurationMs,
            devFixer: devFixerSignal,
          })
        } else {
      ...
      if (req.method === 'POST' && workspaceInviteCreateMatch) {
        ...
        const actor = ensureWorkspaceRole(req, workspaceId, 'admin')
        const idempotencyKey = requireIdempotency(req)
        const replay = getIdempotentReplay({
          key: idempotencyKey,
        ...
        const role = String(body?.role || 'member')
          .trim()
          .toLowerCase() as 'owner' | 'admin' | 'member'
        if (!email) return sendJson(res, 400, { ok: false, error: 'email is required' })
      ...
      if (req.method === 'POST' && url.pathname === '/v1/invites/accept') {
        ...
        if (!accepted.ok) {
          ...
          return sendJson(res, accepted.statusCode || 400, {
            ok: false,
            error: accepted.error || 'invite_accept_failed',
          })
        }
        logSoc2({
          req,
      ...
      if (req.method === 'GET' && url.pathname === '/v1/remote-runs') {
        const workspaceId = String(url.searchParams.get('workspace_id') || '').trim()
        const status = String(url.searchParams.get('status') || '').trim() as
          | 'queued'
          | 'running'
          | 'completed'
          | 'failed'
      ...
      if (req.method === 'POST' && workflowTemplateRunMatch) {
        ...
        logSoc2({
          ...
          details: {
            template_id: run.template.id,
            remote_run_id: remote.id,
            resolved_steps: run.resolved_steps.length,
          },
        })
        await publishWorkspaceEvent({
          workspace_id: run.template.workspace_id,
      ...
      if (req.method === 'POST' && url.pathname === '/v1/runtime/tasks') {
        ...
        if (!workspaceId || !command)
          return sendJson(res, 400, { ok: false, error: 'workspace_id and command are required' })
        ensureWorkspaceRole(req, workspaceId, 'member')
      ...
      if (req.method === 'POST' && url.pathname === '/v1/orchestrator/github/event') {
        ...
        if (event === 'ci') {
          const raw = String(body?.ciStatus || '')
            .trim()
            .toLowerCase()
        ...
        if (eventName === 'pull_request_review' || eventName === 'pull_request_review_comment') {
          ...
          if (!reviewBody || !branchName || !issueId || !repoPath) {
            ...
            return sendJson(res, 400, {
              ok: false,
              error:
                'review webhook requires review/comment body, pull_request head.ref, pull_request number, and repoPath',
            })
          }
          const queued = queueRevisionFromReview({
            workflowId,
      ...
      const agentNameMatch = url.pathname.match(/^\/v1\/agents\/(.+)$/)
      if (req.method === 'GET' && agentNameMatch) {
        const agentName = decodeURIComponent(agentNameMatch[1] || '')
        const agent = getAgent(agentName)
        if (!agent) {
          return sendJson(res, 404, { ok: false, error: 'agent not found' })
        }
        // Increment download count
        incrementDownloads(agentName)
        // Get rating
        const rating = getAgentRating(agentName)
        return sendJson(res, 200, { ...agent, rating })
      }

      // Publish agent
      if (req.method === 'POST' && url.pathname === '/v1/agents/publish') {
        const body = (await readJson(req)) as {
          name?: string
          description?: string
          author?: string
        ...
        if (!body?.name) {
          return sendJson(res, 400, { ok: false, error: 'agent name is required' })
        }
        publishAgent({
          name: body.name,
        ...
        trackServerEvent('agent_published' as any, { agent_name: body.name, author: body.author })
        return sendJson(res, 200, { ok: true, success: true })
      }

      // Rate an agent
      if (req.method === 'POST' && url.pathname === '/v1/agents/rate') {
        const body = (await readJson(req)) as { agent?: string; rating?: number; user_id?: string } | null
        const agentName = String(body?.agent || '').trim()
        const rating = Number(body?.rating)
        if (!agentName || !rating || rating < 1 || rating > 5) {
          return sendJson(res, 400, { ok: false, error: 'agent name and rating (1-5) are required' })
        }
        rateAgent(agentName, rating, body?.user_id)
        return sendJson(res, 200, { ok: true, success: true })
      }

      // Purchase agent (create Stripe checkout session)
      if (req.method === 'POST' && url.pathname === '/v1/agents/purchase') {
        const body = (await readJson(req)) as { agentName?: string; email?: string } | null
        ...
        if (!agentName || !email) {
          return sendJson(res, 400, { ok: false, error: 'agentName and email are required' })
        }
        ...
        if (hasPurchased(agentName, email)) {
          return sendJson(res, 200, { ok: true, purchased: true, message: 'Already purchased' })
        }

        // If free, record purchase
        if (!agent.price || agent.price === 0) {
          recordPurchase(agentName, email, 0)
          return sendJson(res, 200, { ok: true, free: true, purchased: true })
        }

        // Create Stripe checkout session for paid agents
        let stripe: any = null
        try {
          const stripeModule = require('stripe')
          const stripeKey = process.env.STRIPE_SECRET_KEY
