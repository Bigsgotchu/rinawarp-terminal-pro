...
import type { EvidenceBundle, Snapshot } from '../types/index.ts'
...
export class SystemParser {
   ...
   */
  buildEvidence(
    stepOutputs: Record<string, { stdout: string; stderr: string; exitCode: number | null }>
  ): EvidenceBundle {
    ...
    return {
      collectedAt: now,
      raw: Object.fromEntries(Object.entries(stepOutputs).map(([k, v]) => [k, { ...v, stepId: k, durationMs: 0 }])),
      metrics,
      snapshots,
    }
  }
