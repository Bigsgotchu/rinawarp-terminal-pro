...
import { spawn, ExecException } from 'node:child_process'
 ...
 */
export async function run(command: string, options?: TerminalOptions): Promise<TerminalResult> {
 ...
 */
export async function runOrThrow(command: string, options?: TerminalOptions): Promise<string> {
 ...
 */
export function getPlatform(): 'linux' | 'darwin' | 'win32' {
...
export const platform = {
  darwin: {
    ...
    getThermalInfo:
      'sudo powermetrics --samplers=cpu_power -n 1 | grep -E \'(CPU die temperature|CPU power)\' || echo "Thermal info unavailable"',
  ...
  linux: {
    ...
    getThermalInfo:
      'cat /sys/class/thermal/thermal_zone*/temp 2>/dev/null | head -5 || echo "Thermal info unavailable"',
  ...
  win32: {
    ...
    getThermalInfo:
      'wmic /namespace:\\\\root\\wmi PATH MSAcpi_ThermalZoneTemperature GET CurrentTemperature 2>&1 || echo "Thermal info unavailable"',
