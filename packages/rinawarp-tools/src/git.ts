 ...
 */
export async function getLog(repoPath: string, count: number = 10): Promise<GitLogEntry[]> {
 ...
 */
export async function stage(repoPath: string, files: string[] = []): Promise<TerminalResult> {
 ...
 * @returns Promise resolving to command result
 */
export async function commit(repoPath: string, message: string): Promise<TerminalResult> {
...
function parseGitFileList(content: string): string[] {
  return content
    ...
    .map((line) => {
      ...
      return line.replace(/^(new file:|modified:|deleted:)\s*/i, '').replace(/^\s+/, '')
    })
}
