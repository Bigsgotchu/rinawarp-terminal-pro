    ...
    case 'darwin':
      return [
        ...
        },
        {
          tool: 'terminal',
          command:
            'sudo powermetrics --samplers=cpu_power -n 1 2>/dev/null | grep -E \'(CPU die temperature|CPU power)\' || echo "Thermal info via powermetrics unavailable (may require sudo)"',
    ...
    case 'linux':
      return [
        ...
        },
        {
          tool: 'terminal',
          command:
            'cat /sys/class/thermal/thermal_zone*/temp 2>/dev/null | head -5 || echo "Thermal sensors unavailable"',
        ...
        },
        {
          tool: 'terminal',
          command:
            'sensors 2>/dev/null | grep -E \'(Core|temp|Package)\' | head -10 || echo "sensors command unavailable"',
    ...
    case 'win32':
      return [
        ...
        },
        {
          tool: 'terminal',
          command:
            'wmic /namespace:\\\\root\\wmi PATH MSAcpi_ThermalZoneTemperature GET CurrentTemperature 2>&1 || echo "Thermal sensors unavailable"',
 ...
 */
export function parseDiagnostics(results: Array<{ command: string; output: string }>): DiagnosticResult {
