...
const tests = {
  async testMainPage() {
    ...
    if (!response.body.includes('RinaWarp')) {
      throw new Error('Page does not contain RinaWarp branding');
    }
  ...
  async testBetaDownloadPage() {
    ...
    if (!response.body.includes('Beta Testing')) {
      throw new Error('Page does not contain beta testing content');
    }
  ...
  async testResponseTimes() {
    ...
    if (response.responseTime > 5000) {
      throw new Error(`Page load too slow: ${response.responseTime}ms`);
    }
  ...
  async testMetaTags() {
    ...
    if (missing.length > 0) {
      throw new Error(`Missing meta tags: ${missing.map(m => m.tag).join(', ')}`);
    }
  ...
  async testFavicon() {
    try {
      ...
      if (response.statusCode === 404) {
        throw new Error('Favicon not found in common locations');
      }
  ...
  async testRobotsTxt() {
    try {
      ...
      throw new Error('robots.txt not accessible');
    }
  },
  ...
  async testSitemap() {
    try {
      ...
      throw new Error('sitemap.xml not accessible');
    }
  },
  ...
  async testContentIntegrity() {
    ...
    if (missing.length > 0) {
      throw new Error(`Missing critical content: ${missing.join(', ')}`);
    }
