...
async function buildProduction() {
  ...
  if (fs.existsSync(distPath)) {
    ...
    expectedFiles.mac.forEach(f => console.log(`    - ${f} (${getFileSize(path.join(distPath, f))})`));
    
    console.log(`  • Windows: ${expectedFiles.win.length} files`);
    expectedFiles.win.forEach(f => console.log(`    - ${f} (${getFileSize(path.join(distPath, f))})`));
    
    console.log(`  • Linux: ${expectedFiles.linux.length} files`);
    expectedFiles.linux.forEach(f => console.log(`    - ${f} (${getFileSize(path.join(distPath, f))})`));
