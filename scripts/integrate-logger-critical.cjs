...
function hasLoggerImport(content) {
  return content.includes('logger.js') || 
    ...
    if (hasChanges && !alreadyHasLogger) {
      ...
      if (isESModule) {
        ...
        if (lastImport !== -1) {
          ...
          content = content.slice(0, endOfLine + 1) + loggerImport + '\n' + content.slice(endOfLine + 1);
        } else {
        ...
        if (lastRequire !== -1) {
          ...
          if (requireLine.includes('const') || requireLine.includes('let') || requireLine.includes('var')) {
            content = content.slice(0, endOfLine + 1) + loggerImport + '\n' + content.slice(endOfLine + 1);
          }
        } else {
