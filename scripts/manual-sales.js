...
Purchase Date: ${new Date(saleRecord.timestamp).toLocaleDateString()}
  ...
  processManualSale() {
    ...
    console.log(
      `4. Mark sale as completed: node scripts/manual-sales.js complete ${saleRecord.id}`
    );
  ...
  listSales() {
    ...
    files.forEach(filename => {
      ...
      console.log(
        `${saleRecord.id} | ${date} | ${saleRecord.customer.name} | ${saleRecord.customer.plan.toUpperCase()} | ${status}`
      );
    });
  }
}
