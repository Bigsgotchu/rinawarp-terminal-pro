...
function analyzeDownloadFiles() {
  ...
  for (const filename of CONFIG.targetFiles) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      log(`  📦 ${filename}: ${sizeInMB}MB ${tooLarge ? colors.red('(TOO LARGE)') : colors.green('(OK)')}`, 'info');
    } else {
      fileAnalysis.push({
        filename,
        exists: false,
...
function updateDownloadLinks(fileAnalysis) {
  ...
  fileAnalysis.forEach(file => {
    if (file.exists) {
      ...
      const oldHrefPattern = new RegExp(`href="[^"]*${file.filename.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"`, 'g');
...
function createGitHubRelease() {
  ...
  try {
    ...
    const releaseCheck = execSync('gh release view latest', { encoding: 'utf8' });
    log('✅ GitHub release exists', 'success');
    
    // Upload large files to release
    const largeFiles = fs.readdirSync(CONFIG.releasesDir)
...
function main() {
  ...
  try {
    ...
    const vercelConfig = createVercelConfig(fileAnalysis);
