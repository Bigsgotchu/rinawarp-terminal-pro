  ...
  try {
    ...
    if (primaryResult.status >= 200 && primaryResult.status < 400) {
      ...
      analytics.siteHealth.averageResponseTime = 
        ((analytics.siteHealth.averageResponseTime * (analytics.siteHealth.successfulRequests - 1)) + responseTime) / 
        analytics.siteHealth.successfulRequests;
      
      log(`✅ Site healthy - Status: ${primaryResult.status}, Response: ${responseTime.toFixed(2)}ms`, 'success');
    } else {
...
function simulateUserBehavior() {
  ...
  if (Math.random() < 0.3) { // 30% chance of user activity
    ...
    if (Math.random() < 0.15) { // 15% chance of download
      simulateDownload();
    }
  }
}
...
function checkAlertThresholds() {
  ...
  const errorRate = (analytics.siteHealth.failedRequests / analytics.siteHealth.totalRequests) * 100;
  ...
  if (responseTime > CONFIG.thresholds.responseTimeCritical) {
    createAlert('RESPONSE_TIME_CRITICAL', `Response time critically high: ${responseTime.toFixed(2)}ms`, 'critical');
  } else if (responseTime > CONFIG.thresholds.responseTimeWarning) {
    createAlert('RESPONSE_TIME_WARNING', `Response time warning: ${responseTime.toFixed(2)}ms`, 'warning');
  }
  
  // Error rate alerts
  if (errorRate > CONFIG.thresholds.errorRateCritical) {
    createAlert('ERROR_RATE_CRITICAL', `Error rate critically high: ${errorRate.toFixed(2)}%`, 'critical');
  ...
  if (downloadSuccessRate < (100 - CONFIG.thresholds.downloadFailureCritical)) {
    createAlert('DOWNLOAD_CRITICAL', `Download success rate critically low: ${downloadSuccessRate.toFixed(2)}%`, 'critical');
  } else if (downloadSuccessRate < (100 - CONFIG.thresholds.downloadFailureWarning)) {
    createAlert('DOWNLOAD_WARNING', `Download success rate warning: ${downloadSuccessRate.toFixed(2)}%`, 'warning');
  }
}
...
function generateReport() {
  ...
  const errorRate = analytics.siteHealth.totalRequests > 0 ? 
    (analytics.siteHealth.failedRequests / analytics.siteHealth.totalRequests) * 100 : 0;
  ...
  console.log(`   Successful: ${analytics.siteHealth.successfulRequests} (${analytics.siteHealth.uptimePercentage.toFixed(2)}%)`);
  ...
  console.log(`   Windows: ${analytics.downloads.byPlatform.windows.total} (Installer: ${analytics.downloads.byPlatform.windows.installer}, Portable: ${analytics.downloads.byPlatform.windows.portable})`);
  ...
  console.log(`   Desktop: ${analytics.userBehavior.devices.desktop}, Mobile: ${analytics.userBehavior.devices.mobile}, Tablet: ${analytics.userBehavior.devices.tablet}`);
