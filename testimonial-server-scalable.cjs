...
const submissionLimiter = createRateLimiter(
  60 * 60 * 1000,
  5,
  'Too many submissions, please wait an hour'
);
  ...
  try {
    const result = await client.query(
      ...
      [
        ...
        type,
        content,
        metadata.testimonialApproved,
        metadata.displayName,
        metadata.displayCompany,
        metadata.ipAddress,
        metadata.userAgent,
        metadata.referrer,
        metadata.priority,
...
app.get('/health', async (req, res) => {
  try {
    ...
    res.json({
      ...
      testimonials: {
        pending: await pool
          .query('SELECT COUNT(*) FROM testimonials WHERE status = $1', ['pending'])
          .then(r => r.rows[0].count),
        approved: await pool
          .query('SELECT COUNT(*) FROM testimonials WHERE status = $1', ['approved'])
          .then(r => r.rows[0].count),
      },
    });
  } catch (error) {
...
app.post('/admin/login', async (req, res) => {
  try {
    ...
    // Update last login
    await pool.query('UPDATE admin_users SET last_login = CURRENT_TIMESTAMP WHERE id = $1', [
      admin.id,
    ]);
...
const DOMAIN =
  process.env.NODE_ENV === 'production' ? 'https://rinawarptech.com' : `http://localhost:${PORT}`;
