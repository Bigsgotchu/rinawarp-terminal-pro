...
// CLI Configuration
program.version('1.0.0').description('RinaWarp Terminal - Mailchimp Contact Management CLI');
...
program
  ...
  .action(async options => {
    ...
    if (!apiKey || !serverPrefix) {
      ...
      console.log(
        '📝 Server prefix is in your API key (e.g., if key ends with "-us1", server is "us1")'
      );
      ...
      .on('end', async () => {
        ...
        try {
          ...
          for (let i = 0; i < contacts.length; i += batchSize) {
            ...
            console.log(
              `⏳ Importing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(contacts.length / batchSize)}...`
            );
...
program
  ...
  .action(async options => {
    ...
    try {
      ...
      console.log(
        `📋 Contacts in audience (showing ${response.members.length} of ${response.total_items}):\n`
      );
    ...
    try {
      ...
      const csvData = allMembers.map(member => ({
        ...
        signup_date: member.timestamp_signup
          ? new Date(member.timestamp_signup).toISOString().split('T')[0]
          : '',
...
program
  ...
  .action(() => {
    ...
    console.log(
      '3. mailchimp-cli import-contacts -f ./marketing/master-contacts-for-mailchimp.csv'
    );
    ...
    console.log(
      '• mailchimp-cli add-tags -t "launch-supporter,engaged-high" -e "user@example.com"\n'
    );
