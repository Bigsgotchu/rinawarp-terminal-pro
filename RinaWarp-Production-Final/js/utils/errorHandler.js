 ...
 * user-friendly notifications, detailed logging, and Sentry integration
 */

import { captureException, captureMessage, addSentryBreadcrumb } from './sentry.js';
...
export class ErrorHandler {
  ...
  /**
   * Report error to monitoring systems (Sentry)
   */
  reportError(errorInfo, recovery) {
    try {
      // Add breadcrumb for context
      addSentryBreadcrumb(
        `Error handled: ${errorInfo.message}`,
        'error',
        recovery.type === 'critical' ? 'error' : 'warning'
      );
      ...
      captureException(error, {
        tags: {
          ...
          errorType: recovery.type,
          severity: this.getSeverityFromRecovery(recovery),
          recovered: recovery.success.toString(),
        ...
        level: this.getSentryLevel(recovery),
      });

      console.log('📊 Error reported to Sentry');
    } catch (sentryError) {
      console.warn('⚠️ Failed to report error to Sentry:', sentryError.message);
    }
  }

  /**
   * Get Sentry severity level from recovery info
   */
  getSeverityFromRecovery(recovery) {
    if (recovery.type === 'critical') return 'critical';
    if (!recovery.success) return 'high';
    if (recovery.fallback) return 'medium';
    return 'low';
  }

  /**
   * Get Sentry level from recovery info
   */
  getSentryLevel(recovery) {
    if (recovery.type === 'critical') return 'fatal';
    if (!recovery.success) return 'error';
    if (recovery.fallback) return 'warning';
    return 'info';
  }
