...
export class AIManager {
  ...
  async loadConfig() {
    // Default configuration - Use Groq as default (FREE & FAST!)
    this.config = {
      provider: 'groq',
  ...
  getAPIKey() {
    // Check localStorage first for stored API keys
    const provider = this.config.provider;
    const storedKey = localStorage.getItem(`ai-key-${provider}`);
    if (storedKey) {
      return storedKey;
    }
  ...
  async generateResponse(prompt, options = {}) {
    ...
    try {
      switch (this.provider) {
        case 'groq':
          return await this.generateGroqResponse(prompt, config);
        case 'claude':
  ...
  async generateGroqResponse(prompt, config) {
    const systemPrompt =
      'You are RinaWarp AI, an intelligent assistant integrated into RinaWarp Terminal. You help developers with coding, terminal commands, debugging, and technical questions. Be concise, helpful, and focus on practical solutions. When providing code, use appropriate syntax highlighting.';
  ...
  async setProvider(provider, apiKey) {
    const validProviders = ['groq', 'claude', 'openai', 'google'];
    ...
    return [
      {
        ...
        description: '🚀 FREE ultra-fast Llama models - Perfect for coding!',
        pricing: 'FREE',
        speed: 'Ultra Fast',
        recommended: true,
      },
