...
export class EnhancedPerformanceMonitor {
  ...
  setupMutationObserver() {
    ...
    const mutationCallback = (mutations) => {
      ...
      if (addedNodes + removedNodes + attributeChanges > 10) {
        console.debug(`DOM Changes - Added: ${addedNodes}, Removed: ${removedNodes}, Attributes: ${attributeChanges}`);
      }
    };
  ...
  processPerformanceMetrics(metrics) {
    ...
    // Process FID
    if (metrics.fid) {
      this.trackWebVital({ name: 'FID', value: metrics.fid.processingStart - metrics.fid.startTime });
    }
