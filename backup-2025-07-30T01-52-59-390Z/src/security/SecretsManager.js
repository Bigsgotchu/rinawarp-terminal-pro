...
class SecretsManager {
  ...
  getOrCreateEncryptionKey() {
    ...
    try {
      ...
      console.warn('Could not read existing encryption key');
    }
    ...
    try {
      ...
      console.log('✅ Generated new secrets encryption key');
    } catch (error) {
      console.warn('Could not save encryption key to file');
    }
  ...
  setSecret(key, value, metadata = {}) {
    try {
      ...
      console.error(`❌ Failed to store secret '${key}':`, error.message);
      return false;
    }
  }
  ...
  loadSecrets() {
    ...
    try {
      if (fs.existsSync(secretsPath)) {
        ...
        console.log(`🔐 Loaded ${this.secrets.size} secrets from storage`);
      }
    } catch (error) {
      console.warn('Could not load existing secrets:', error.message);
    }
  }
  ...
  saveSecrets() {
    ...
    try {
      ...
      console.error('Could not save secrets:', error.message);
    }
  }
  ...
  initializeFromEnv() {
    ...
    if (initialized > 0) {
      console.log(`🔐 Initialized ${initialized} secrets from environment variables`);
    }
  }
