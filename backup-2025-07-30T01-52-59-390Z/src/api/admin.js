...
secretsRouter.get('/', requirePermission(PERMISSIONS.READ_USERS), (req, res) => {
  const secretsManager = getSecretsManager();
  res.json(secretsManager.listSecrets());
});

secretsRouter.post('/', requirePermission(PERMISSIONS.WRITE_USERS), (req, res) => {
...
secretsRouter.delete('/:key', requirePermission(PERMISSIONS.WRITE_USERS), (req, res) => {
