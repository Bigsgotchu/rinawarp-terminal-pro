...
class AgentMode extends EventEmitter {
  ...
  async initialize() {
    console.log('🤖 Initializing RinaWarp Agent Mode...');
...
You can use function calls to interact with the terminal and file system. Always explain what you're doing and why.`;
  ...
  async registerBuiltInFunctions() {
    ...
    console.log(`📚 Registered ${this.functionRegistry.getFunctionCount()} agent functions`);
  }
...
class AgentSession {
  ...
  async close() {
    ...
    console.log(`Agent session ${this.id} closed after ${Math.round(duration / 1000)}s`);
    console.log(`Messages: ${this.messageCount}, Function calls: ${this.functionCallCount}`);
  }
}
