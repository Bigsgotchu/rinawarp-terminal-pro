...
class GitIntegration {
      ...
      if (status.hasRemote) {
        try {
          const aheadBehind = await this.executeGitCommand([
            ...
            `${status.branch}...origin/${status.branch}`
          ], directory);
  ...
  async getBranches(directory) {
    try {
      const result = await this.executeGitCommand(['branch', '--format=%(refname:short)'], directory);
      if (result.exitCode === 0) {
    ...
    lines.forEach(line => {
      if (line.startsWith('@@')) {
        ...
        if (diffLine.type === 'context' || diffLine.type === 'deletion') {
          diffLine.oldLineNumber = currentHunk.oldStart + currentHunk.lines.filter(
            l => l.type === 'context' || l.type === 'deletion'
          ).length;
        }
        
        if (diffLine.type === 'context' || diffLine.type === 'addition') {
          diffLine.newLineNumber = currentHunk.newStart + currentHunk.lines.filter(
