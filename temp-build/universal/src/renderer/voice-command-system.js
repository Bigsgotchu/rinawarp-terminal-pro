...
export class VoiceCommandSystem {
  ...
  initializeSpeechSynthesis() {
    ...
    const loadVoices = () => {
      ...
      ) || voices.find(v => v.lang.startsWith(this.options.language.split('-')[0])) || voices[0];
  ...
  isDirectCommand(text) {
    const commonCommands = [
      'ls', 'cd', 'pwd', 'cat', 'echo', 'grep', 'find', 'ps', 'top', 'df', 'free',
      'git', 'npm', 'yarn', 'node', 'python', 'java', 'make', 'cmake', 'docker',
  ...
  /**
     ...
     */
  handleShowHelp() {
    const helpText = `Available voice commands include:
