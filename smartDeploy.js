...
try {
  ...
  console.error(
    colors.red('⚠️ Could not detect active project. Try running: firebase use default')
  );
  process.exit(1);
}
...
console.log(
  colors.gray('   URL: https://console.firebase.google.com/project/rinawarp-terminal/hosting')
);
try {
...
setTimeout(() => {
  ...
  for (const url of hostedURLs) {
    try {
      ...
      const curlResult = execSync(`curl -s -w "HTTPSTATUS:%{http_code}" -L "${url}"`, {
        encoding: 'utf8',
      });
