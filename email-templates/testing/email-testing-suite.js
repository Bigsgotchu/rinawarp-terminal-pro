...
class EmailTestingSuite {
    ...
    async loadTemplate() {
        const templateContent = fs.readFileSync(
            path.join(__dirname, this.templatePath), 
            'utf8'
        );
                ...
                const renderingIssues = await page.evaluate(() => {
                    ...
                    textElements.forEach(el => {
                        ...
                        if (style.fontSize === '0px' || style.visibility === 'hidden') {
                            issues.push(`Invisible text: ${el.tagName} with content: ${el.textContent.slice(0, 50)}...`);
                        }
                    });
    ...
    async analyzePerformance() {
        ...
        if (templateSize > 102400) {
            this.results.performanceMetrics.recommendations.push('Consider optimizing HTML size (current: ' + Math.round(templateSize / 1024) + 'KB)');
        }
        ...
        if (images.some(img => !img.hasAlt)) {
            this.results.performanceMetrics.recommendations.push('Add alt text to all images for accessibility');
        }
    ...
    async analyzeSpamScore() {
        ...
        const spamWords = [
            ...
            'no cost', 'risk-free', 'money back', 'incredible deal'
        ];
        ...
        // Generate recommendations
        if (spamScore > 0) {
            this.results.spamScore.recommendations.push('Review flagged issues to improve deliverability');
        }
    ...
    async generateABTestVariations() {
        ...
        variations.forEach(variation => {
            fs.writeFileSync(
                path.join(variationsDir, `variation-${variation.id}.html`),
                variation.html
            );
        });
    ...
    async generateReport() {
        ...
        report.summary.failed = allTests.filter(t => t.status === 'FAIL' || t.status === 'ERROR').length;
