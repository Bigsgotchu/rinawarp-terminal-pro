...
async function demonstrateRinaVoice() {
  ...
  try {
    ...
    const { RinaVoiceIntegration } = await import('./src/voice-enhancements/rina-voice-integration.js');
  ...
  const demos = [
    ...
    
    {
      ...
      action: () => {
        ...
        moods.forEach(mood => {
          ...
          console.log(`  ${kleur.blue(mood)}: rate=${config.rate}, pitch=${config.pitch}, volume=${config.volume}`);
        });
      }
    },
    
    {
      ...
      action: () => {
        ...
        console.log(`  Cache size: ${kleur.cyan(cacheStatus.cacheSize)}/${cacheStatus.maxCacheSize}`);
    ...
    
    {
      ...
      action: () => {
        ...
        Object.entries(integrationStatus).forEach(([key, value]) => {
          if (key !== 'rinaVoiceStatus') { // Skip nested object for cleaner display
            const displayValue = typeof value === 'object' ? JSON.stringify(value) : value;
            console.log(`    ${kleur.yellow(key)}: ${kleur.white(displayValue)}`);
          }
        });
