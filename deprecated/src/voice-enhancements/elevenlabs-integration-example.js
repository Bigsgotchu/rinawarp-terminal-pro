...
export class ElevenLabsIntegrationManager {
  ...
  integrateSystems() {
    ...
    if (this.rinaIntegration?.rinaVoice) {
      const originalRinaSpeak = this.rinaIntegration.rinaVoice.speak.bind(this.rinaIntegration.rinaVoice);
  ...
  async announceError(errorMessage, severity = 'normal') {
    ...
    try {
      await this.elevenLabsProvider.speak(
        `Error: ${errorMessage}. Let me help you fix this.`,
        {
          mood,
          priority: 'high',
          volume: 0.8
    ...
    try {
      return await this.elevenLabsProvider.speakWithFallback(text, {
        ...
        onFallback: (fallbackType) => {
          ...
          window.dispatchEvent(new CustomEvent('elevenlabs-fallback-used', {
  ...
  getStatus() {
    ...
    return {
      ...
      healthSummary: elevenLabsStatus ? {
        ...
        fallbacksAvailable: elevenLabsStatus.fallbackSystems?.synthesis || elevenLabsStatus.fallbackSystems?.rinaClips,
        cacheEfficiency: elevenLabsStatus.metrics?.cacheHits / Math.max(1, elevenLabsStatus.metrics?.totalRequests) || 0,
        errorRate: elevenLabsStatus.metrics?.failedRequests / Math.max(1, elevenLabsStatus.metrics?.totalRequests) || 0
      } : null
    };
  }
  ...
  getPerformanceMetrics() {
    ...
    return {
      ...
      cacheStats: {
        ...
        cacheHitRate: (status.metrics.cacheHits / Math.max(1, status.metrics.totalRequests) * 100).toFixed(1) + '%'
      },
      fallbackStats: {
        synthesisAvailable: status.fallbackSystems.synthesis,
        rinaClipsAvailable: status.fallbackSystems.rinaClips,
        fallbackUsageRate: (status.metrics.fallbackHits / Math.max(1, status.metrics.totalRequests) * 100).toFixed(1) + '%'
      }
    };
  }
