...
class DailyReportGenerator {
  ...
  async generateDailyReport(date = new Date()) {
    ...
    metrics.openRate =
      metrics.uniqueOpens > 0 ? ((metrics.uniqueOpens / metrics.emailOpens) * 100).toFixed(2) : 0;
    metrics.clickRate =
      metrics.uniqueOpens > 0 ? ((metrics.uniqueClicks / metrics.uniqueOpens) * 100).toFixed(2) : 0;
    metrics.conversionRate =
      metrics.uniqueClicks > 0
        ? ((metrics.uniqueConversions / metrics.uniqueClicks) * 100).toFixed(2)
        : 0;
  ...
  getTrafficSources(conversions) {
    ...
    return Object.entries(sources)
      .map(([key, data]) => ({
        ...
        percentage:
          conversions.length > 0 ? ((data.conversions / conversions.length) * 100).toFixed(1) : 0,
      }))
      .sort((a, b) => b.conversions - a.conversions);
  }
...
<body>
      ...
      blocks: [
        ...
        },
        {
          ...
          text: {
            ...
            text: `*Top Campaigns:*\n${report.campaignMetrics
              .slice(0, 3)
              .map(c => `• ${c.campaignId}: ${c.conversions} conversions`)
              .join('\n')}`,
          },
        },
      ],
  ...
  scheduleDailyReports() {
    ...
    console.log(
      `📅 Daily reports scheduled for ${this.reportConfig.schedule.time} (${timeUntilNext}ms from now)`
    );
