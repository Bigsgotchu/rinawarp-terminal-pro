...
⏰ Time: ${new Date(conversion.timestamp).toLocaleString()}`,
  ...
  async sendToCustomEndpoint(conversion) {
    await axios.post(
      this.analyticsConfig.customEndpoint.url,
      {
        event: 'beta_signup',
        data: conversion,
      },
  ...
  generateDailyReport(date = new Date()) {
    ...
    const report = {
      ...
      metrics: {
        ...
        conversionRate:
          opens.length > 0 ? ((conversions.length / opens.length) * 100).toFixed(2) : 0,
      },
  ...
  saveDailyReport(date = new Date()) {
    ...
    const reportPath = path.join(
      __dirname,
      `daily-report-${date.toISOString().split('T')[0]}.json`
    );
...
if (require.main === module) {
  ...
  const trackingCode = tracker.generateConversionTrackingCode(
    'beta-signup-form',
    'beta-launch-2024'
  );
