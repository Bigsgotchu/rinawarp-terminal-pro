...
async function handleFixConfirmation(msg: string): Promise<{ role: 'rina'; text: string }[]> {
  ...
  return [{ role: 'rina', text: formatOutcome(verified.outcome) }]
}
...
function buildSystemContext(context?: ChatContext): string {
  ...
  return [profileSummary ? `## Agent Profile\n${profileSummary}` : '', rulesBlock].filter(Boolean).join('\n\n')
}
...
async function handleSystemDoctor(intent: string, context?: ChatContext): Promise<{ role: 'rina'; text: string }[]> {
  ...
  const intentForModel = systemContext ? `${systemContext}\n\n## User Intent\n${safeIntent}` : safeIntent
