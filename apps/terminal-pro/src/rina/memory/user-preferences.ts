...
export class UserMemoryStore {
  ...
  recordToolUse(toolId: string): void {
    ...
    this.memory.lastUsedTools = [
      toolId,
      ...this.memory.lastUsedTools.filter(t => t !== toolId)
    ].slice(0, this.maxToolHistory);
  ...
  recordCommand(command: string, success: boolean): void {
    ...
    if (success) {
      this.memory.successfulCommands = [
        command,
        ...this.memory.successfulCommands.filter(c => c !== command)
      ].slice(0, this.maxHistoryLength);
    } else {
      this.memory.failedCommands = [
        command,
        ...this.memory.failedCommands.filter(c => c !== command)
      ].slice(0, this.maxHistoryLength);
    }
