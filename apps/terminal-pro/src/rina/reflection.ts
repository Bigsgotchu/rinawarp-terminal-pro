...
export class ReflectionEngine {
  ...
  async analyzeStep(step: RinaPlanStep, result: StepExecutionResult): Promise<ReflectionInsight> {
    ...
    const avgDuration = history.length > 0 ? history.reduce((a, b) => a + b, 0) / history.length : 5000 // Default 5 seconds
    ...
    if (duration > avgDuration * 1.5 && duration > 0) {
      feedback.push(
        `Step "${step.description}" took longer than expected (${duration}ms vs avg ${Math.round(avgDuration)}ms).`
      )
  ...
  async reflectOnTask(
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      const history = this.performanceHistory.get(step.id) || []
      const avgDuration = history.length > 0 ? history.reduce((a, b) => a + b, 0) / history.length : 5000
