...
export function safetyCheck(command: string, mode: ExecutionMode): SafetyCheckResult {
  // ALWAYS check for blocked commands first, regardless of mode
  // This prevents dangerous commands from executing in ANY mode
  ...
  // Explain mode: show what would happen but still allow the explanation
  // Confirmation is not required since we're just explaining
  if (mode === 'explain') {
    return {
      blocked: false,
      requiresConfirmation: false,
      reason: 'Explain mode: showing what would happen (command is safe)',
    }
  }

  // Assist mode: require confirmation for potentially dangerous commands
  ...
  // Auto mode: allow execution without confirmation (user has opted in to automation)
  return {
    blocked: false,
    requiresConfirmation: false,
  }
