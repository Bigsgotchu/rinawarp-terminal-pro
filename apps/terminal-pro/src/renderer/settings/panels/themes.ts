...
export async function mountThemesPanel(container: HTMLElement): Promise<void> {
  ...
  const render = () => {
    ...
    const quickGrid = quick.length
      ? `<div class="rw-grid">${quick.map((t) => renderThemeCard(t, selected.id)).join('')}</div>`
      : ''
    const recentGrid = recents.length
      ? `<div class="rw-grid">${recents.map((t) => renderThemeCard(t, selected.id)).join('')}</div>`
      : ''
