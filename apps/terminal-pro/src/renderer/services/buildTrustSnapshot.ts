...
export function buildTrustSnapshotFromState(state: WorkbenchState): string {
  ...
  return [
    ...
    `Avg run duration: ${state.analytics.executionMetrics.runsCompleted > 0 ? Math.round(state.analytics.executionMetrics.totalDurationMs / state.analytics.executionMetrics.runsCompleted / 1000) : 0}s`,
    `Verification pass rate: ${state.analytics.executionMetrics.verificationPassCount + state.analytics.executionMetrics.verificationFailCount > 0 ? Math.round((state.analytics.executionMetrics.verificationPassCount / (state.analytics.executionMetrics.verificationPassCount + state.analytics.executionMetrics.verificationFailCount)) * 100) : 0}%`,
    `Rollbacks: ${state.analytics.executionMetrics.rollbackCount}`,
