...
// Use createRequire to import CommonJS electron module in ESM context
...
import { registerIpcHandlers, setDaemonFunctions } from './main/ipc/index.js'
import { registerIpcHandlers as registerRinaIpcHandlers } from './main/ipc/router.js'
import { registerAgentHandlers } from './main/ipc/agent.js'
import { registerPtyHandlers } from './main/pty/ptyController.js'
import { registerPlanExecutor } from './main/agent/planExecutor.js'
...
import {
  ...
  type ToolEvent,
} from '@rinawarp/core/enforcement/index.js'
...
const ALLOW_LOCAL_ENGINE_FALLBACK =
  fallbackEnv == null || fallbackEnv === '' ? true : /^(1|true|yes)$/i.test(fallbackEnv)
...
function loadTeamDb(): TeamDb {
  return (
    readJsonIfExists<TeamDb>(TEAM_FILE()) ?? {
...
const operationalMemory = {
  ...
  getRecent(category: string, limit = 5): any[] {
    ...
    return arr.sort((a: any, b: any) => b.timestamp - a.timestamp).slice(0, limit)
  },
}
...
function getPtyModule(): Promise<PtyModule | null> {
  if (!ptyModulePromise) {
    ptyModulePromise = import('node-pty').then((mod) => mod as unknown as PtyModule).catch(() => null)
  }
...
function closePtyForWebContents(webContentsId: number): void {
  ...
  // Two-stage process termination: SIGTERM then SIGKILL
  const ptyProcess = session.proc
  const webContents = BrowserWindow.fromWebContentsId(webContentsId)

  // Track if process is still running after SIGTERM
  let processStillRunning = true

  // Set up exit listener to track if process terminates
  ...
  setTimeout(() => {
    ...
    ptyProcess.onExit(() => {}) // Replace with no-op

    // Notify renderer that PTY has been terminated
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('rina:pty:terminated', { webContentsId })
    }
  }, 2000)
...
async function diagnoseHotLinux(): Promise<{
  ...
  const topProcesses = await runCommandOnce(TOP_CPU_CMD_SAFE.replace('head -15', 'head -n 15'), 8000).catch(
    (e) => `Unable to read processes: ${String(e)}`
  )
...
async function runCommandOnceViaEngine(command: string, timeoutMs: number): Promise<string> {
  ...
  if (!result?.success) {
    const err = (result as { error?: string })?.error ?? 'Command failed'
    throw new Error(err)
  }
...
function createWindow() {
  ...
  // Set daemon functions for IPC handlers before registration
  setDaemonFunctions({
    daemonStatus,
    daemonTasks,
    daemonTaskAdd,
  ...
  // Register IPC handlers now that we have a valid window
  ...
  // Send thinking stream events to renderer
...
async function startStreamingStepViaEngine(args: {
  ...
  if (!profileGate.ok) {
    const error = (profileGate as { message: string }).message
  ...
  const error = cancelled
    ...
    : report.ok
      ...
      : lastResult && !lastResult.success
        ? ((lastResult as { error?: string }).error ?? 'Execution failed')
...
async function shareCreateForIpc(args: {
  ...
  const requiredRole =
    args?.requiredRole && ['owner', 'operator', 'viewer'].includes(args.requiredRole) ? args.requiredRole : 'viewer'
...
async function executeStepStreamForIpc(args: {
  ...
  if (!profileGate.ok) {
    safeSend(eventSender, 'rina:stream:end', {
      ...
      code: null,
      cancelled: false,
      error: (profileGate as { message: string }).message,
...
async function doctorExecuteFixForIpc(plan: any, confirmed: boolean, confirmationText: string) {
  ...
  for (const step of steps) {
    ...
    if (!profileGate.ok) {
      return { ok: false, haltedBecause: (profileGate as { message: string }).message, steps: [] }
    }
...
function formatDiagnosisForChat(diagnosis: any): string {
  ...
  if (diagnosis.differential?.length) {
    msg += `\n\n**Other possibilities**: ${diagnosis.differential
      .slice(0, 3)
      .map((d: any) => `${d.label} (${Math.round(d.probability * 100)}%)`)
      .join(', ')}`
  }
...
app.whenReady().then(async () => {
  // Auto-start daemon when Electron launches
  try {
  ...
  // Register RinaWarp IPC handlers
  registerRinaIpcHandlers()
  registerPtyHandlers()
  registerPlanExecutor()
  registerAgentHandlers()

  // Analytics IPC handler for renderer -> main process funnel tracking
  ipcMain.handle('rina:analytics:funnel', async (_event: any, step: string, properties?: Record<string, unknown>) => {
    try {
  ...
  // Agent IPC handlers for hybrid Chat + CLI + IDE UI
  ipcMain.handle('agent:send', async (_event: any, text: string) => {
    // Forward to daemon for processing
    try {
      const response = await fetch('http://localhost:5055/agent/send', {
        method: 'POST',
      ...
      return { text: `Error: ${error}`, actions: [] }
    }
  })

  ipcMain.handle('agent:execute', async (_event: any, cmd: string) => {
    // Execute command via PTY
    try {
      return { ok: true, output: `Executed: ${cmd}` }
    } catch (error) {
      return { ok: false, error: String(error) }
    }
  })

  // Terminal execution for CLI blocks in chat
  ipcMain.handle('terminal:run', async (_event: any, command: string) => {
    console.log('[terminal:run] Executing:', command)
    return new Promise((resolve) => {
      // Use child_process exec to run the command
      const { exec } = require('child_process')
      exec(command, { timeout: 60000 }, (error: Error | null, stdout: string, stderr: string) => {
        console.log('[terminal:run] Result:', { error, stdout: stdout?.slice(0, 100), stderr: stderr?.slice(0, 100) })
        if (error) {
  ...
  ipcMain.handle('agent:loadSession', async (_event: any, id: string) => {
    return { ok: true, sessionId: id }
  })

  ipcMain.removeHandler('agent:executePlan')
  ipcMain.handle('agent:executePlan', async (_event: any, id: string) => {
    return { ok: true, planId: id }
  })

  ipcMain.handle('workspace:getFiles', async (_event: any, root: string) => {
    // Return mock files for now - implement with actual fs
    return ['main.ts', 'package.json', 'README.md']
  })

  ipcMain.handle('workspace:readFile', async (_event: any, filePath: string) => {
    // Return mock content - implement with actual fs
