...
const TEMPLATES: Record<SmallTalkIntent, Record<Tone, string[]>> = {
  greeting: {
    professional: [
      ...
      "Hey. What do you want to tackle?",
    ],
    friendly: [
...
function pick<T>(arr: T[], avoid: Set<T>): T {
  ...
  return candidates.length ? candidates[Math.floor(Math.random() * candidates.length)] : arr[Math.floor(Math.random() * arr.length)];
}
...
export async function generatePersonalityReply(args: {
  ...
  if (persona.emojiLevel === 0) text = text.replace(/[^\x00-\x7F]/g, "").replace(/\s+/g, " ").trim();
