...
export function buildAgentContext(request: RinaAgentRequest, deps: RinaAgentDeps = {}): AgentState {
  const allWorkspaceFacts: WorkspaceFact[] = [
    ...(request.workspaceKnowledge?.architecture || []),
    ...(request.workspaceKnowledge?.dependencies || []),
    ...(request.workspaceKnowledge?.facts || []),
  ];
  ...
  return {
    ...
    toolCallsUsed: 0,
    workspaceKnowledge: request.workspaceKnowledge
      ? {
          architecture: request.workspaceKnowledge.architecture,
          dependencies: request.workspaceKnowledge.dependencies,
          facts: allWorkspaceFacts,
        }
      : undefined,
  };
}
...
function fixtureFileContent(filePath: string): string {
  const base = path.basename(filePath);
  if (/\.(c|m)?js$/i.test(base)) {
    return [
      "// Fixture content generated by Rina to satisfy the package script target.",
      "// Replace this with the intended test logic once the real coverage is known.",
      "console.log(\"Fixture script completed successfully.\");",
      "",
    ].join("\n");
  }
  if (/\.ts$/i.test(base)) {
