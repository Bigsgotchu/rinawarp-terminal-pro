...
export function registerTeamIpc(args: {
  ...
  ipcMain.handle('rina:team:listInvites', async (_event, payload?: { includeSecrets?: boolean }) =>
    args.listInvites(payload)
  )
  ipcMain.handle('rina:team:acceptInvite', async (_event, payload: { inviteCode?: string }) =>
    args.acceptInvite(payload)
  )
