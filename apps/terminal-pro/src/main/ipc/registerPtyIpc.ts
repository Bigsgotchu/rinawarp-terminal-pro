...
import { trackSessionStart, trackSessionEnd } from '../telemetry.js'
...
export function registerPtyIpc(args: {
  ipcMain: IpcMain
  ctx: AppContext
  ...
  args.ipcMain.handle('rina:pty:start', async (event, payload?: { cols?: number; rows?: number; cwd?: string }) => {
    ...
    proc.onExit(({ exitCode, signal }: { exitCode: number; signal?: number }) => {
      ...
      // Track session end for telemetry
      trackSessionEnd()
    })
  ...
  args.ipcMain.handle('rina:pty:write', async (event, data: string) => {
  ...
  args.ipcMain.handle('rina:pty:resize', async (event, cols: number, rows: number) => {
    const session = args.ptySessions.get(event.sender.id)
    if (!session) return { ok: false, error: 'PTY not started' }
  ...
  args.ipcMain.handle('rina:pty:metrics', async (event) => {
    const session = args.ptySessions.get(event.sender.id)
    if (!session) return { ok: false, error: 'PTY not started' }
    return {
      ok: true,
  ...
  args.ipcMain.handle('rina:pty:stop', async (event) => {
    const webContentsId = event.sender.id
    const session = args.ptySessions.get(webContentsId)
