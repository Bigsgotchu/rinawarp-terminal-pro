...
export function registerDiagnosticsIpc(args: {
  ...
  args.ipcMain.handle("rina:renderer:error", async (_event, payload: { kind?: string; message?: string; extra?: string }) => {
    try {
      fs.appendFileSync(args.rendererErrorsFile(), `${JSON.stringify({
