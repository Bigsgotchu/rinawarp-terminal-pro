...
export function loadProjectRules(projectRoot: string, opts?: { maxBytesPerFile?: number; parentLevels?: number }): ProjectRulesBundle {
  ...
  for (const rr of roots) {
    for (const c of candidates) {
      ...
      try {
        ...
        warnings.push(`Failed to read ${path.relative(root, fp) || c.rel}: ${error instanceof Error ? error.message : String(error)}`);
        continue;
      }
